...
describe("Hook Origin Tracking", () => {
  it("tracks hooks by origin kit", () => {
    ...
    expect(result.hooksByOrigin.get("engineer")).toContain(
      "node ~/.claude/scripts/foo.cjs",
    );
  });
  ...
  it("separates hooks by different kits", () => {
    ...
    const result1 = mergeSettings(engineerHooks, dest, {
      sourceKit: "engineer",
    });
    ...
    const result2 = mergeSettings(marketingHooks, result1.merged, {
      sourceKit: "marketing",
    });
  ...
  it("preserves existing hooks without sourceKit in origin map", () => {
    ...
    expect(marketingHooks).not.toContain(
      "node ~/.claude/scripts/existing-hook.cjs",
    );
  });
