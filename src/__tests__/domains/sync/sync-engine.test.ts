...
describe("SyncEngine", () => {
  ...
  describe("validateSyncPath", () => {
    ...
    it("rejects null bytes", async () => {
      await expect(validateSyncPath(claudeDir, "file\0.txt")).rejects.toThrow(
        "null byte",
      );
    });
    ...
    it("rejects overly long paths", async () => {
      ...
      await expect(validateSyncPath(claudeDir, longPath)).rejects.toThrow(
        "Path too long",
      );
    });
    ...
    it("rejects traversal patterns", async () => {
      await expect(
        validateSyncPath(claudeDir, "../etc/passwd"),
      ).rejects.toThrow("Path traversal not allowed");
      await expect(
        validateSyncPath(claudeDir, "foo/../../../etc/passwd"),
      ).rejects.toThrow("Path traversal not allowed");
    });
    ...
    it("rejects symlinks that escape base directory", async () => {
      ...
      await expect(
        validateSyncPath(claudeDir, "escape-link/secret.txt"),
      ).rejects.toThrow("Symlink escapes base directory");
    });
  ...
  describe("loadFileContent", () => {
    ...
    it("throws for non-existent files", async () => {
      await expect(
        SyncEngine.loadFileContent("/nonexistent/file.txt"),
      ).rejects.toThrow("Cannot read file for sync");
    });
