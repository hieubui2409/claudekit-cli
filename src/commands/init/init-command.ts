 ...
 */
async function installAdditionalKit(
  baseCtx: InitContext,
  kitType: KitType,
): Promise<InitContext> {
...
export async function initCommand(
  ...
  try {
    ...
    if (
      ...
      for (const pendingKit of kitsToInstall) {
        try {
          ...
          if (ctx.cancelled) {
            logger.warning(
              `Installation of ${AVAILABLE_KITS[pendingKit].name} was cancelled`,
            );
            break;
          }
    ...
    if (ctx.resolvedDir) {
      await maybeShowConfigUpdateNotification(
        ctx.resolvedDir,
        ctx.options.global,
      );
    }
