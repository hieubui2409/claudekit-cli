...
} from "@/domains/ui/conflict-summary.js";
...
export async function handleMerge(ctx: InitContext): Promise<InitContext> {
  ...
  if (ctx.options.only && ctx.options.only.length > 0) {
    ...
    if (!updateEverything) {
      includePatterns = await ctx.prompts.promptDirectorySelection(
        ctx.options.global,
      );
  ...
  if (customClaudeFiles.length > 0) {
    ...
    logger.success(
      `Protected ${customClaudeFiles.length} custom .claude file(s)`,
    );
  }
  ...
  if (!ctx.options.fresh && (await pathExists(ctx.claudeDir))) {
    ...
    if (legacyDetection.isLegacy && releaseManifest) {
      logger.info(
        "Legacy installation detected - migrating to ownership tracking...",
      );
  ...
  if (CommandsPrefix.shouldApplyPrefix(ctx.options)) {
    ...
    if (ctx.options.dryRun) {
      const { OwnershipDisplay } =
        await import("@/domains/ui/ownership-display.js");
