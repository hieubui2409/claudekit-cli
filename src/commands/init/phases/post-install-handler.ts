...
export async function handlePostInstall(
  ...
  if (ctx.options.global) {
    ...
    if (await pathExists(claudeMdSource)) {
      if (!(await pathExists(claudeMdDest))) {
        ...
        logger.debug(
          "CLAUDE.md already exists in global directory (preserved)",
        );
      }
    }
  }
  ...
  if (installSkills) {
    const { handleSkillsInstallation } =
      await import("@/services/package-installer/package-installer.js");
    const skillsDir = PathResolver.buildSkillsPath(
      ctx.resolvedDir,
      ctx.options.global,
    );
  ...
  if (!ctx.isNonInteractive) {
    ...
    const existingConfig = checkExistingGeminiConfig(
      ctx.resolvedDir,
      ctx.options.global,
    );
    ...
    if (geminiInstalled && !existingConfig.exists && mcpConfigExists) {
      const geminiPath = ctx.options.global
