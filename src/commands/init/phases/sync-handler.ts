...
export async function handleSync(ctx: InitContext): Promise<InitContext> {
  ...
  const claudeDir = ctx.options.global
    ? resolvedDir
    : join(resolvedDir, ".claude");

  // Check if .claude directory exists
  if (!(await pathExists(claudeDir))) {
    logger.error("Cannot sync: no .claude directory found");
    ctx.prompts.note(
      "Run 'ck init' without --sync to install first.",
  ...
  if (!kitType) {
    ...
    if (engineerMeta && marketingMeta) {
      ...
      if (!ctx.isNonInteractive) {
        ...
        logger.error(
          "Multiple kits installed. Please specify --kit engineer or --kit marketing",
        );
        return { ...ctx, cancelled: true };
      }
...
function getLockTimeout(): number {
  ...
  if (Number.isNaN(parsed) || parsed < 0) {
    logger.warning(
      `Invalid CK_SYNC_LOCK_TIMEOUT "${envValue}", using default (30s)`,
    );
  ...
  if (timeoutMs < MIN_LOCK_TIMEOUT_MS) {
    logger.warning(
      `CK_SYNC_LOCK_TIMEOUT too low (${parsed}s), using minimum (5s)`,
    );
    return MIN_LOCK_TIMEOUT_MS;
  }
  if (timeoutMs > MAX_LOCK_TIMEOUT_MS) {
    logger.warning(
      `CK_SYNC_LOCK_TIMEOUT too high (${parsed}s), using maximum (300s)`,
    );
...
async function acquireSyncLock(global: boolean): Promise<() => Promise<void>> {
      ...
      if ((err as NodeJS.ErrnoException).code === "EEXIST") {
        ...
        try {
          ...
          if (lockAge > STALE_LOCK_THRESHOLD_MS) {
            logger.warning(
              `Removing stale sync lock (age: ${Math.round(lockAge / 1000)}s)`,
            );
  ...
  throw new Error(
    "Failed to acquire sync lock: another sync operation may be in progress",
  );
}
...
export async function executeSyncMerge(ctx: InitContext): Promise<InitContext> {
  ...
  try {
    ...
    const upstreamDir = ctx.options.global
      ? join(ctx.extractDir, ".claude")
      : ctx.extractDir;

    logger.info("Analyzing file changes...");

    // Create sync plan
    const plan = await SyncEngine.createSyncPlan(
      trackedFiles,
      ctx.claudeDir,
      upstreamDir,
    );
    ...
    if (plan.autoUpdate.length === 0 && plan.needsReview.length === 0) {
      ctx.prompts.note(
        "All files are up to date or user-owned.",
        "No Changes Needed",
      );
      return { ...ctx, cancelled: true };
    }
        ...
        try {
          ...
          try {
            ...
            if (errCode === "EROFS" || errCode === "EACCES") {
              logger.warning(
                `Cannot create directory ${file.path}: ${errCode}`,
              );
              updateFailed++;
              continue;
            }
          ...
          if (errCode === "EACCES" || errCode === "EPERM") {
            logger.warning(
              `Permission denied: ${file.path} - check file permissions`,
            );
            updateFailed++;
    ...
    if (plan.needsReview.length > 0 && !ctx.isNonInteractive) {
      logger.info(
        `${plan.needsReview.length} file(s) need interactive review...`,
      );
      ...
      for (const file of plan.needsReview) {
        ...
        // Generate hunks
        const hunks = SyncEngine.generateHunks(
          currentContent,
          newContent,
          file.path,
        );
        ...
        const result = await MergeUI.mergeFile(
          file.path,
          currentContent,
          newContent,
          hunks,
      ...
      if (plan.autoUpdate.length > 0) {
        console.log(
          pc.green(`  ✓ ${plan.autoUpdate.length} file(s) auto-updated`),
        );
      }
      if (totalApplied > 0) {
      ...
      if (plan.skipped.length > 0) {
        console.log(
          pc.dim(`  ─ ${plan.skipped.length} user-owned file(s) unchanged`),
        );
      }
...
function displaySyncPlan(plan: SyncPlan): void {
  ...
  if (plan.autoUpdate.length > 0) {
    console.log(
      pc.green(`  ${plan.autoUpdate.length} file(s) will be auto-updated`),
    );
  ...
  if (plan.needsReview.length > 0) {
    console.log(
      pc.yellow(`  ${plan.needsReview.length} file(s) need interactive review`),
    );
  ...
  if (plan.skipped.length > 0) {
    console.log(
      pc.dim(`  ${plan.skipped.length} user-owned file(s) will be skipped`),
    );
  }
