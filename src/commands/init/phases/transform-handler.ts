...
export async function handleTransforms(ctx: InitContext): Promise<InitContext> {
  ...
  if (ctx.options.global) {
    ...
    const transformResult = await transformPathsForGlobalInstall(
      ctx.extractDir,
      {
  ...
  const hasCustomFolders =
    foldersConfig.docs !== DEFAULT_FOLDERS.docs ||
    foldersConfig.plans !== DEFAULT_FOLDERS.plans;

  if (hasCustomFolders) {
    logger.info(
      `Using custom folder names: docs=${foldersConfig.docs}, plans=${foldersConfig.plans}`,
    );
    const folderTransformResult = await transformFolderPaths(
      ctx.extractDir,
      foldersConfig,
      {
        verbose: logger.isVerbose(),
