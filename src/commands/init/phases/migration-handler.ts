...
export async function handleMigration(ctx: InitContext): Promise<InitContext> {
  ...
  const currentSkillsDir = PathResolver.buildSkillsPath(
    ctx.resolvedDir,
    ctx.options.global,
  );

  if (
    !(await pathExists(newSkillsDir)) ||
    !(await pathExists(currentSkillsDir))
  ) {
    return ctx;
  }
  ...
  if (
    ...
    const migrationResult = await SkillsMigrator.migrate(
      newSkillsDir,
      currentSkillsDir,
      {
    ...
    if (!migrationResult.success) {
      logger.warning(
        "Skills migration encountered errors but continuing with update",
      );
    }
  } else {
