...
export async function removeInstallations(
  ...
  for (const installation of installations) {
    ...
    if (options.dryRun) {
      const label = options.kit
        ? `${installation.type} (${options.kit} kit)`
        : installation.type;
      displayDryRunPreview(analysis, label);
      if (analysis.remainingKits.length > 0) {
        log.info(
          `Remaining kits after uninstall: ${analysis.remainingKits.join(", ")}`,
        );
      }
      continue;
    }
    ...
    try {
      ...
      for (const item of analysis.toDelete) {
        ...
        if (await pathExists(filePath)) {
          ...
          cleanedDirs += await cleanupEmptyDirectories(
            filePath,
            installation.path,
          );
        }
      ...
      if (options.kit && analysis.remainingKits.length > 0) {
        await ManifestWriter.removeKitFromManifest(
          installation.path,
          options.kit,
        );
      }
      ...
      try {
        ...
        if (remaining.length === 0) {
          ...
          logger.debug(
            `Removed empty installation directory: ${installation.path}`,
          );
        }
      } catch {
