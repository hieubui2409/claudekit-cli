...
export async function cleanupEmptyDirectories(
  ...
  while (
    currentDir !== installationRoot &&
    currentDir.startsWith(installationRoot)
  ) {
    try {
...
export async function analyzeInstallation(
  ...
  const uninstallManifest = await ManifestWriter.getUninstallManifest(
    installation.path,
    kit,
  );
  ...
  if (uninstallManifest.isMultiKit && kit && metadata?.kits?.[kit]) {
    ...
    for (const trackedFile of kitFiles) {
      ...
      if (uninstallManifest.filesToPreserve.includes(trackedFile.path)) {
        result.toPreserve.push({
          path: trackedFile.path,
          reason: "shared with other kit",
        });
        continue;
      }
...
export function displayDryRunPreview(
  ...
  if (analysis.toDelete.length > 0) {
    console.log(
      pc.red(pc.bold(`Files to DELETE (${analysis.toDelete.length}):`)),
    );
  ...
  if (analysis.toPreserve.length > 0) {
    console.log(
      pc.green(pc.bold(`Files to PRESERVE (${analysis.toPreserve.length}):`)),
    );
    const showPreserve = analysis.toPreserve.slice(0, 10);
    for (const item of showPreserve) {
      console.log(
        `  ${pc.green("✓")} ${item.path} ${pc.gray(`(${item.reason})`)}`,
      );
    }
