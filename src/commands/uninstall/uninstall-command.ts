...
function displayInstallations(
  ...
  const scopeLabel =
    scope === "all" ? "all" : scope === "local" ? "local only" : "global only";

  prompts.note(
    installations
      .map((i) => `  ${i.type === "local" ? "Local " : "Global"}: ${i.path}`)
      .join("\n"),
  ...
  const options: { value: UninstallScope; label: string; hint: string }[] = [
    {
      value: "local",
      label: "Local only",
      hint: "Remove from current project (.claude/)",
    },
    {
      value: "global",
      label: "Global only",
      hint: "Remove from user directory (~/.claude/)",
    },
...
export async function uninstallCommand(
  ...
  try {
    ...
    // 10. Confirm deletion
    if (!validOptions.yes) {
      const kitLabel = validOptions.kit
        ? ` (${validOptions.kit} kit only)`
        : "";
