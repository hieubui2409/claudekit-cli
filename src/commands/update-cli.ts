...
const KIT_UPDATE_REMINDER_HEADER =
  "Note: 'ck update' only updates the CLI tool itself.";
...
async function displayKitUpdateReminder(): Promise<void> {
  try {
    ...
    lines.push("");
    lines.push(
      "To update your ClaudeKit content (skills, commands, workflows):",
    );

    if (hasLocal && localVersion) {
      lines.push(
        `  ${picocolors.cyan(pad(cmdLocal))}  Update local project (${localVersion})`,
      );
      const localCheck = versionCheckResults.get(localVersion);
      if (localCheck?.updateAvailable) {
        const indent = " ".repeat(maxCmdLen + 4);
        lines.push(
          `${indent}${picocolors.green(`→ ${localCheck.latestVersion} available!`)}`,
        );
      }
    } else {
      lines.push(
        `  ${picocolors.cyan(pad(cmdLocal))}  Initialize in current project`,
      );
    }
    ...
    if (hasGlobal && globalVersion) {
      ...
      if (globalCheck?.updateAvailable) {
        const indent = " ".repeat(maxCmdLen + 4);
        lines.push(
          `${indent}${picocolors.green(`→ ${globalCheck.latestVersion} available!`)}`,
        );
      }
    } else {
      lines.push(
        `  ${picocolors.cyan(pad(cmdGlobal))}  Initialize global ~/.claude`,
      );
    }
...
export async function updateCliCommand(
  ...
  try {
    ...
    if (opts.release && opts.release !== "latest") {
      ...
      targetVersion = await NpmRegistryClient.getBetaVersion(
        PACKAGE_NAME,
        opts.registry,
      );
      if (!targetVersion) {
        ...
        logger.warning(
          "No beta version found. Using latest stable version instead.",
        );
    ...
    if (comparison > 0 && !opts.release) {
      ...
      outro(
        `[+] Current version (${currentVersion}) is newer than latest (${targetVersion})`,
      );
      return;
    }
    ...
    try {
      ...
      outro(
        `[+] Update completed. Please restart your terminal to use CLI ${targetVersion}`,
      );
      await displayKitUpdateReminder();
    }
  } catch (error) {
