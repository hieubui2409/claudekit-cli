  ...
  async addTrackedFilesBatch(
    ...
    const tasks = files.map((file) =>
      limit(async (): Promise<boolean> => {
        try {
          const checksum = await OwnershipChecker.calculateChecksum(
            file.filePath,
          );
    ...
    if (failed > 0) {
      logger.warning(
        `Failed to track ${failed} of ${total} files (check debug logs for details)`,
      );
    }
 ...
 */
export function buildFileTrackingList(
  options: BuildFileTrackingOptions,
): FileTrackInfo[] {
  const {
    installedFiles,
    claudeDir,
...
export async function trackFilesWithProgress(
  ...
  const trackingSpinner = createSpinner(
    `Tracking ${filesToTrack.length} installed files...`,
  );
