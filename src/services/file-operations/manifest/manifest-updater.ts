...
export async function writeManifest(
  ...
  const kit: KitType =
    kitType || (/\bmarketing\b/i.test(kitName) ? "marketing" : "engineer");
  ...
  try {
    ...
    const metadata: Metadata = {
      ...
      installedAt: otherKitsExist
        ? (existingMetadata.installedAt ?? installedAt)
        : installedAt,
    ...
    logger.debug(
      `Wrote manifest for kit "${kit}" with ${trackedFiles.length} tracked files`,
    );
  } finally {
    // Always release lock
    if (release) {
