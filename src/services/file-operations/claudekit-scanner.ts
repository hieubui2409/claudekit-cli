...
export async function scanClaudeKitDirectory(
  directoryPath: string,
): Promise<ComponentCounts> {
  ...
  try {
    ...
    if (items.includes("commands")) {
      ...
      counts.commands = commandFiles.filter((file) =>
        file.endsWith(".md"),
      ).length;
    }
    ...
    if (items.includes("workflows")) {
      ...
      counts.workflows = workflowFiles.filter((file) =>
        file.endsWith(".md"),
      ).length;
    }
...
export async function getClaudeKitSetup(
  ...
  if (await pathExists(projectClaudeDir)) {
    ...
    setup.project.metadata = await readClaudeKitMetadata(
      join(projectClaudeDir, "metadata.json"),
    );
