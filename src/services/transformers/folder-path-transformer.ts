...
import {
  collectDirsToRename,
  renameFolders,
} from "./folder-transform/folder-renamer.js";
import {
...
export async function transformFolderPaths(
  ...
  const needsTransform =
    folders.docs !== DEFAULT_FOLDERS.docs ||
    folders.plans !== DEFAULT_FOLDERS.plans;
  ...
  result.foldersRenamed = await renameFolders(
    dirsToRename,
    extractDir,
    options,
  );
  ...
  const transformedFiles = await transformFileContents(
    extractDir,
