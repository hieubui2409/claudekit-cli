 ...
 */
export function addSymlinkSkip(
  entry: string,
  accumulator: CleanupAccumulator,
): void {
...
export function logCleanupSummary(
  ...
  if (dryRun) {
    logger.info(
      `DRY RUN complete: would delete ${deletedCount}, preserve ${preservedCount}`,
    );
  } else {
    logger.success(
      `Cleanup complete: deleted ${deletedCount}, preserved ${preservedCount}`,
    );
  }
  ...
  if (preservedCount > 0 && !dryRun) {
    ...
    preserved
      .slice(0, 5)
      .forEach((r) => logger.info(`  - ${r.path} (${r.reason})`));
