...
import type {
  GeminiLinkOptions,
  GeminiLinkResult,
} from "./gemini-mcp/linker-core.js";
import {
...
export async function linkGeminiMcpConfig(
  ...
  if (!mcpConfigPath) {
    return {
      ...
      method: "symlink",
      error:
        "No MCP config found. Create .mcp.json or ~/.claude/.mcp.json first.",
    };
  }
...
export async function processGeminiMcpLinking(
  ...
  if (result.success) {
    if (result.method === "symlink") {
      logger.success(
        `Gemini MCP linked: ${settingsPath} → ${result.targetPath}`,
      );
      logger.info("MCP servers will auto-sync with your Claude config.");
    } else if (result.method === "merge") {
      logger.success(
        "Gemini MCP config updated (merged mcpServers, preserved your settings)",
      );
