...
export async function isPackageInstalled(
  ...
  if (isCIEnvironment()) {
    logger.info(
      `CI environment detected: skipping network check for ${packageName}`,
    );
    return false;
  }
  ...
  try {
    ...
    try {
      ...
      const caseInsensitiveMatch = stdout
        .toLowerCase()
...
export async function getPackageVersion(
  ...
  if (isCIEnvironment()) {
    logger.info(
      `CI environment detected: skipping version check for ${packageName}`,
    );
    return null;
  }
...
export async function installPackageGlobally(
  ...
  try {
    ...
    logger.success(
      `${displayName} ${version ? `v${version} ` : ""}installed successfully`,
    );
