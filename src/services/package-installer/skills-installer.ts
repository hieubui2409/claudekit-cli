...
export async function installSkillsDependencies(
  ...
  if (isNonInteractive() && !skipConfirm) {
    logger.info(
      "Running in non-interactive mode. Skipping skills installation.",
    );
  ...
  try {
    ...
    logger.info(
      `  Platform: ${platform === "win32" ? "Windows (PowerShell)" : "Unix (bash)"}`,
    );
    ...
    if (platform === "linux") {
      ...
      if (needsSudo) {
        if (withSudo) {
          ...
          logger.info("");
          logger.warning(
            "System packages skipped (not included without --with-sudo):",
          );
          logger.info("  • ffmpeg - Video/audio processing");
          logger.info("  • imagemagick - Image editing & conversion");
          logger.info("");
    ...
    if (exitCode === EXIT_CODE_PARTIAL_SUCCESS) {
      ...
      logger.info("");
      logger.success(
        "Core functionality is available despite some package failures.",
      );
  ...
  try {
    ...
    if (skillsResult.success) {
      if (skillsResult.version === PARTIAL_INSTALL_VERSION) {
        logger.success(
          "Skills core dependencies installed (some optional packages skipped)",
        );
      } else {
      ...
      logger.warning(
        `Skills installation incomplete: ${skillsResult.error || "Unknown error"}`,
      );
      logger.info(
        "You can install skills dependencies manually. See INSTALLATION.md",
      );
    }
  } catch {
