...
export async function createNewSettingsWithMerge(
  ...
  if (
    ...
    return {
      success: false,
      method: "merge",
      error: "MCP config has no valid mcpServers object",
    };
  }
  ...
  try {
    ...
    logger.debug(
      `Created new Gemini settings with mcpServers: ${geminiSettingsPath}`,
    );
...
export async function mergeGeminiSettings(
  ...
  if (!geminiSettings) {
    return {
      success: false,
      method: "merge",
      error: "Failed to read existing Gemini settings",
    };
  }
