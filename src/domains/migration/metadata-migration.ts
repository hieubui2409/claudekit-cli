 ...
 */
export async function detectMetadataFormat(
  claudeDir: string,
): Promise<MetadataFormatDetection> {
...
export async function migrateToMultiKit(
  ...
  try {
    ...
    logger.info(
      `Migrated metadata from legacy format to multi-kit (detected: ${legacyKit})`,
    );
 ...
 */
export function getKitMetadata(
  metadata: Metadata,
  kit: KitType,
): KitMetadata | null {
