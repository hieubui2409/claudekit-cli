...
export class OwnershipPrompts {
  ...
  static async confirmDestructiveOperation(
    ...
    if (isNonInteractive()) {
      ...
      logger.info(
        "Use interactive mode or explicit force flags for destructive operations",
      );
      return false;
    }
   ...
   */
  static async promptModifiedFileDecision(
    modifiedFiles: string[],
  ): Promise<ModifiedFileDecision> {
    ...
    if (isNonInteractive()) {
      logger.info(
        `Non-interactive mode: keeping ${modifiedFiles.length} modified file(s)`,
      );
      return "keep";
    }
    ...
    // Show modified files
    clack.log.warn(
      pc.yellow(`Found ${modifiedFiles.length} modified file(s):`),
    );
   ...
   */
  static async promptFileSelection(
    ...
    if (isNonInteractive()) {
      logger.info(
        `Non-interactive mode: preserving all ${preserveResults.length} file(s)`,
      );
  ...
  static async confirmForceOverwrite(fileCount: number): Promise<boolean> {
    ...
    if (isNonInteractive()) {
      logger.warning(
        `Non-interactive mode: rejecting force overwrite of ${fileCount} file(s)`,
      );
      logger.info(
        "Use interactive mode or explicit force flags for destructive operations",
      );
  ...
  static async promptLegacyMigration(): Promise<boolean> {
    ...
    const proceed = await clack.confirm({
      message:
        "Would you like to migrate to the new ownership tracking system?",
      initialValue: true,
    });
