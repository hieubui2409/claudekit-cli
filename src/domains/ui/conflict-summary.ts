...
export function displayConflictSummary(summary: ConflictSummary): void {
  const totalUpdated =
    summary.files.updated.length +
    summary.hooks.updated.length +
    summary.mcp.updated.length;
  const totalKept =
    summary.files.kept.length +
    summary.hooks.kept.length +
    summary.mcp.kept.length;
  ...
  if (logger.isVerbose() && totalUpdated > 0) {
    ...
    for (const file of summary.files.updated) {
      console.log(
        `  ${pc.dim("-")} ${file.relativePath}: ${pc.green(file.winner)} won`,
      );
    }
    for (const hook of summary.hooks.updated) {
      const shortCmd =
        hook.command.length > 40
          ? `${hook.command.slice(0, 40)}...`
          : hook.command;
      console.log(
        `  ${pc.dim("-")} hook: ${shortCmd} → ${pc.green(hook.winner)}`,
      );
    }
    for (const mcp of summary.mcp.updated) {
      console.log(
        `  ${pc.dim("-")} mcp: ${mcp.serverName} → ${pc.green(mcp.winner)}`,
      );
    }
  }
...
function formatWinners(items: FileConflictInfo[]): string {
  ...
  for (const item of items) {
    const kit =
      item.winner === "incoming" ? item.incomingKit : item.existingKit;
