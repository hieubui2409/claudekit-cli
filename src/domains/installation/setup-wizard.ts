...
export async function runSetupWizard(
  ...
  if (hasGlobalConfig && Object.keys(globalEnv).length > 0) {
    clack.log.success(
      "Global config detected - values will be inherited automatically",
    );
  }
  ...
  for (const config of ESSENTIAL_CONFIGS) {
    ...
    if (hasGlobalValue) {
      const maskedValue = config.mask
        ? `${globalValue.slice(0, 8)}...`
        : globalValue;
    ...
    const result = await clack.text({
      ...
      validate: (value) => {
        ...
        if (
          value &&
          config.validate &&
          !validateApiKey(value, config.validate)
        ) {
  ...
  if (values.GEMINI_API_KEY) {
    const additionalKeys = await promptForAdditionalGeminiKeys(
    ...
    if (totalKeys > 1) {
      clack.log.success(
        `✓ Configured ${totalKeys} Gemini API keys for rotation`,
      );
    }
  }
 ...
 */
async function promptForAdditionalGeminiKeys(
  primaryKey: string,
): Promise<string[]> {
  ...
  const wantMore = await clack.confirm({
    message:
      "Add additional API keys for rotation? (recommended for high usage)",
    initialValue: false,
  });
