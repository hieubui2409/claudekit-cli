...
export class CopyExecutor {
  ...
  setManifest(manifest: ReleaseManifest | null): void {
    ...
    if (manifest && this.selectiveMerger?.hasManifest()) {
      ...
      if (this.claudeDir && this.installingKit) {
        this.selectiveMerger.setMultiKitContext(
          this.claudeDir,
          this.installingKit,
        );
      }
    ...
    for (const file of files) {
      ...
      if (await pathExists(destPath)) {
        ...
        if (this.userConfigChecker.ignores(normalizedRelativePath)) {
          logger.debug(
            `User config file exists and will be preserved: ${normalizedRelativePath}`,
          );
          continue;
        }
  ...
  async copyFiles(sourceDir: string, destDir: string): Promise<void> {
    ...
    for (const file of files) {
      ...
      if (this.fileScanner.shouldNeverCopy(normalizedRelativePath)) {
        logger.debug(
          `Skipping security-sensitive file: ${normalizedRelativePath}`,
        );
        skippedCount++;
        continue;
      }
      ...
      if (this.userConfigChecker.ignores(normalizedRelativePath)) {
        ...
        logger.debug(
          `Copying user config (first-time): ${normalizedRelativePath}`,
        );
      }
    ...
    if (parts.length > 0) {
      ...
      logger.success(
        `Copied ${copiedCount} file(s), skipped ${skippedCount} protected file(s)`,
      );
    }
  }
