...
import {
  type SettingsJson,
  SettingsMerger,
} from "@/domains/config/settings-merger.js";
...
export class SettingsProcessor {
  ...
  private initTracker(): void {
    if (this.projectDir) {
      this.tracker = new InstalledSettingsTracker(
        this.projectDir,
        this.isGlobal,
        this.kitName,
      );
  ...
  async processSettingsJson(
    ...
    try {
      ...
      if (this.isGlobal) {
        ...
        const projectDirVar = isWindows()
          ? '"%CLAUDE_PROJECT_DIR%"'
          : '"$CLAUDE_PROJECT_DIR"';
        transformedSource = this.transformClaudePaths(
          sourceContent,
          projectDirVar,
        );
        if (transformedSource !== sourceContent) {
      ...
      if (destExists && !this.forceOverwriteSettings) {
        ...
        try {
          ...
          if (this.forceOverwriteSettings && destExists) {
            logger.debug(
              "Force overwrite enabled, replaced settings.json completely",
            );
  ...
  private async selectiveMergeSettings(
    ...
    try {
      ...
      logger.warning(
        "Failed to parse source settings.json, falling back to overwrite",
      );
      // Re-format to ensure consistent 2-space indentation
    ...
    if (mergeResult.conflictsDetected.length > 0) {
      logger.warning(
        `Duplicate hooks skipped: ${mergeResult.conflictsDetected.length}`,
      );
    }
    ...
    if (
      this.tracker &&
      (mergeResult.newlyInstalledHooks.length > 0 ||
        mergeResult.newlyInstalledServers.length > 0)
    ) {
   ...
   */
  private async readAndNormalizeGlobalSettings(
    destFile: string,
    ...
    try {
      ...
      if (normalized !== content) {
        logger.debug(
          "Normalized $CLAUDE_PROJECT_DIR paths to $HOME in existing global settings",
        );
      }
  ...
  private transformClaudePaths(content: string, prefix: string): string {
    ...
    if (/\.claude\/[^\s"']*[;`$&|><]/.test(content)) {
      logger.warning(
        "Potentially unsafe characters detected in .claude/ paths",
      );
      throw new Error(
        "Settings file contains potentially unsafe path characters",
      );
    }
    ...
    const jsonSafePrefix = prefix.includes('"')
      ? prefix.replace(/"/g, '\\"')
      : prefix;
