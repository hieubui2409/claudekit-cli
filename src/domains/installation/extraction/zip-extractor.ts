...
export class ZipExtractor {
  ...
  private async tryNativeUnzip(
    ...
    return new Promise((resolve) => {
      ...
      mkdir(destDir, { recursive: true })
        ...
        .catch((err: Error) => {
          logger.debug(
            `Failed to create directory for native unzip: ${err.message}`,
          );
          resolve(false);
        });
    });
  ...
  async extract(
    ...
    try {
      ...
      const nativeSuccess = await this.tryNativeUnzip(
        archivePath,
        tempExtractDir,
      );
      ...
      if (!nativeSuccess) {
        ...
        await extractZip(
          archivePath,
          zipOptions as Parameters<typeof extractZip>[1],
        );
      ...
      if (entries.length === 1) {
        ...
        if (rootStat.isDirectory()) {
          ...
          logger.debug(
            `Root directory '${rootEntry}' contains: ${rootContents.join(", ")}`,
          );
