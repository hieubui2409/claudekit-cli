...
Check disk space and directory permissions.`,
    ...
    const tempDir = await fs.promises.mkdtemp(
      path.join(this.tempBaseDir, `ck-git-${kit.repo}-`),
    );
    ...
    try {
      ...
      try {
        ...
        execSync(
          `git clone --depth 1 --branch "${tag}" ${quotedUrl} ${quotedDir}`,
          {
        ...
        if (
          ...
          logger.debug(
            `Shallow clone failed for tag ${tag}, trying full clone...`,
          );
      ...
      await fs.promises
        .rm(tempDir, { recursive: true, force: true })
        .catch((err) =>
          logger.debug(`Failed to cleanup temp dir ${tempDir}: ${err.message}`),
        );
      ...
      if (
        ...
        (stderr.includes("Permission denied") ||
          stderr.includes("Host key verification"))
      ) {
        logger.debug(
          "SSH clone failed, user may need to add SSH key to GitHub",
        );
        throw new Error(
