...
import {
  ...
  FileDownloader,
} from "./download/file-downloader.js";
...
export class DownloadManager {
  ...
  async extractArchive(
    ...
    const slowExtractionWarning = setTimeout(() => {
      ...
      if (isMacOS()) {
        logger.debug(
          "Slow extraction detected on macOS - Spotlight indexing may be interfering",
        );
      }
    ...
    try {
      ...
      if (detectedType === "tar.gz") {
        await this.tarExtractor.extract(
          archivePath,
          destDir,
          this.shouldExclude,
          this.sizeTracker,
        ...
        await this.zipExtractor.extract(
          archivePath,
          destDir,
          this.shouldExclude,
          this.sizeTracker,
