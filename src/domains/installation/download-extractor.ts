...
export async function downloadAndExtract(
  ...
  try {
    ...
    const canPrompt =
      isNonInteractive !== undefined ? !isNonInteractive : process.stdin.isTTY;
  ...
  switch (result.method) {
    ...
    case "token":
      ...
      if (result.token) {
        ...
        logger.info(
          `Token set, retrying download (attempt ${attempt}/${MAX_AUTH_RETRIES})...`,
        );
        ...
        try {
          ...
          if (isAuthError(error)) {
            logger.warning(
              "Token authentication failed. Please check your token.",
            );
            return handleAuthErrorInteractively(
              error,
              release,
              kit,
              exclude,
...
async function downloadViaGitClone(
  ...
  logger.verbose("Git clone complete", {
...
async function downloadViaApi(
  ...
  try {
    ...
    if (downloadInfo.type === "asset") {
      logger.warning(
        "Asset download failed, falling back to GitHub tarball...",
      );
      const tarballInfo = {
