...
export class SelectiveMerger {
  ...
  async shouldCopyFile(
    ...
    try {
      ...
      if (this.claudeDir && this.installingKit) {
        ...
        if (installed.exists) {
          ...
          logger.debug(
            `File ${relativePath} tracked by ${installed.ownerKit} but missing on disk`,
          );
        }
      }
    ...
    if (this.claudeDir && this.installingKit) {
      ...
      if (installed.exists && installed.checksum && installed.ownerKit) {
        ...
        if (installed.checksum === manifestEntry.checksum) {
          ...
          logger.debug(
            `Shared identical: ${relativePath} (owned by ${installed.ownerKit})`,
          );
          return {
            changed: false,
        ...
        if (incomingTimestamp && existingTimestamp) {
          ...
          if (Number.isNaN(incomingTime) || Number.isNaN(existingTime)) {
            logger.debug(
              `Invalid timestamp for ${relativePath}, falling back to version`,
            );
            ...
            logger.debug(
              `Shared tie: ${relativePath} - same timestamp, keeping existing`,
            );
            return {
              changed: false,
              reason: "shared-older",
    ...
    if (destStat.size !== manifestEntry.size) {
      logger.debug(
        `Size differs for ${relativePath}: ${destStat.size} vs ${manifestEntry.size}`,
      );
      return {
        changed: true,
        reason: "size-differ",
