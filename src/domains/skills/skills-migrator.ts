...
export {
  copySkillDirectory,
  executeMigration,
} from "./migrator/migration-executor.js";
  ...
  static async migrate(
    ...
    try {
      ...
      if (options.interactive) {
        ...
        const shouldMigrate =
          await SkillsMigrationPrompts.promptMigrationDecision(detection);
        ...
        // Show preview
        await SkillsMigrationPrompts.showMigrationPreview(
        ...
        const customizationStrategy =
          await SkillsMigrationPrompts.promptCustomizationHandling(
            customizations.filter((c) => c.isCustomized),
          );

        // Filter mappings based on strategy
        if (customizationStrategy === "skip") {
          detection.skillMappings = detection.skillMappings.filter(
            (mapping) => {
              const customization = customizations.find(
                (c) => c.skillName === mapping.skillName,
              );
      ...
      if (options.backup && !options.dryRun) {
        ...
        result.backupPath = await SkillsBackupManager.createBackup(
          currentSkillsDir,
          claudeDir,
        );
      ...
      if (result.backupPath && !options.dryRun) {
        ...
        try {
          await SkillsBackupManager.restoreBackup(
            result.backupPath,
            currentSkillsDir,
          );
