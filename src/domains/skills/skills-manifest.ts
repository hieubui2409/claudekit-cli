...
export class SkillsManifestManager {
  ...
  static async generateManifest(skillsDir: string): Promise<SkillsManifest> {
    ...
    if (!(await pathExists(skillsDir))) {
      throw new SkillsMigrationError(
        `Skills directory does not exist: ${skillsDir}`,
      );
    }
    ...
    logger.debug(
      `Generated manifest with ${skills.length} skills (${structure} structure)`,
    );
    return manifest;
  }
  ...
  static async writeManifest(
    ...
    const manifestPath = join(
      skillsDir,
      SkillsManifestManager.MANIFEST_FILENAME,
    );
   ...
   */
  private static async detectStructure(
    skillsDir: string,
  ): Promise<"flat" | "categorized"> {
    ...
    if (structure === "flat") {
      ...
      for (const category of categories) {
        if (
          ...
          const skillEntries = await readdir(categoryPath, {
            withFileTypes: true,
          });
  ...
  static compareManifests(
    oldManifest: SkillsManifest,
    newManifest: SkillsManifest,
  ): string[] {
    const changedSkills: string[] = [];
    const oldSkillsMap = new Map(
      oldManifest.skills.map((s) => [s.name, s.hash]),
    );
