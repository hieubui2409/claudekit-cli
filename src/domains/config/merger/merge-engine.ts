...
export function mergeMcp(
  ...
  if (sourceMcp.servers) {
    ...
    const destTimestamps =
      options?.installedSettings?.mcpServerTimestamps ?? {};
      ...
      if (serverName in destServers) {
        ...
        if (sourceTs && destTs) {
          ...
          if (sourceTime > destTime) {
            ...
            result.mcpServersOverwritten =
              (result.mcpServersOverwritten ?? 0) + 1;
          ...
          if (sourceTime < destTime) {
            ...
            logger.debug(
              `Preserved MCP server (existing newer): ${serverName}`,
            );
            continue;
          }
        ...
        const userModified =
          ...
          JSON.stringify(destServers[serverName]);
        if (userModified) {
          result.mcpServersPreserved++;
...
export function mergeSettings(
  ...
  // Merge hooks
  if (source.hooks) {
    result.merged.hooks = mergeHooks(
      source.hooks,
      destination.hooks || {},
      result,
      options,
