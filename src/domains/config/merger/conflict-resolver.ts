 ...
 */
function wasCommandInstalled(
  command: string,
  installedHooks: string[],
): boolean {
  const normalizedCommand = normalizeCommand(command);
  return installedHooks.some(
...
function dedupeDestinationEntries(entries: (HookConfig | HookEntry)[]): {
  ...
  for (const entry of entries) {
    ...
    const uniqueCommands = commands.filter(
      (cmd) => !seenCommands.has(normalizeCommand(cmd)),
    );
    ...
    if (uniqueCommands.length === 0) {
      ...
      removedCount++;
      logger.verbose(
        `Removing duplicate hook entry: ${commands[0]?.slice(0, 50)}...`,
      );
      continue;
    }
    ...
    if (
      "hooks" in entry &&
      entry.hooks &&
      uniqueCommands.length < commands.length
    ) {
...
export function mergeHookEntries(
  ...
  const { deduped: dedupedDest, removedCount } =
    dedupeDestinationEntries(destEntries);

  if (removedCount > 0) {
    logger.info(
      `Cleaned up ${removedCount} duplicate hook(s) from existing settings`,
    );
  }
  ...
  for (const entry of sourceEntries) {
    ...
    if (sourceMatcher && matcherIndex.has(sourceMatcher)) {
      ...
      const isFullyDuplicated =
        commands.length > 0 &&
        commands.every((cmd) => existingCommands.has(normalizeCommand(cmd)));
      ...
      if (!isFullyDuplicated && hasNonRemovedCommands) {
        ...
        if ("hooks" in entry && entry.hooks && userRemovedCommands.length > 0) {
          const filteredHooks = entry.hooks.filter(
            (h) =>
              !h.command || !wasCommandInstalled(h.command, installedHooks),
          );
          filteredEntry = { ...entry, hooks: filteredHooks };
        } else if (
          "command" in entry &&
          wasCommandInstalled(entry.command, installedHooks)
        ) {
        ...
        for (const cmd of commands) {
          ...
          if (
            !existingCommands.has(normalizedCmd) &&
            !wasCommandInstalled(cmd, installedHooks)
          ) {
 ...
 */
function tagHooksWithOrigin(
  entry: HookConfig | HookEntry,
  kit: string,
): HookConfig | HookEntry {
