...
export class ConfigManager {
   ...
   */
  private static getProjectConfigDir(
    projectDir: string,
    global: boolean,
  ): string {
  ...
  static async migrateNestedConfig(globalDir: string): Promise<boolean> {
    ...
    if (existsSync(correctPath)) {
      logger.debug(
        "Config already exists at correct location, skipping migration",
      );
      return false;
    }
    ...
    if (existsSync(incorrectPath)) {
      try {
        logger.info(
          "Migrating .ck.json from nested location to correct location...",
        );
