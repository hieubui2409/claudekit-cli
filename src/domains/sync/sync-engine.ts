  ...
  while (depth < maxDepth) {
    try {
      ...
      if (rel.startsWith("..") || isAbsolute(rel)) {
        throw new Error(
          `Symlink chain escapes base directory at depth ${depth}: ${path}`,
        );
      }
 ...
 */
async function validateSyncPath(
  basePath: string,
  filePath: string,
): Promise<string> {
  // Reject empty paths
...
export class SyncEngine {
  ...
  static async createSyncPlan(
    ...
    for (const file of trackedFiles) {
      ...
      const currentChecksum =
        await OwnershipChecker.calculateChecksum(localPath);
   ...
   */
  private static applyHunksManually(
    content: string,
    hunks: FileHunk[],
  ): string {
