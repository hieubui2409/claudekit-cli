...
export class MergeUI {
  ...
  static showExtendedContext(
    ...
    const endLine = Math.min(
      lines.length,
      hunk.oldStart + hunk.oldLines - 1 + contextLines,
    );
    ...
    for (let i = startLine; i < endLine; i++) {
      ...
      const isInHunk =
        i >= hunk.oldStart - 1 && i < hunk.oldStart - 1 + hunk.oldLines;
  ...
  static async mergeFile(
    ...
    console.log(
      pc.dim(
        `${hunks.length} change${hunks.length === 1 ? "" : "s"} to review\n`,
      ),
    );
  ...
  static displayMergeSummary(
    ...
    if (applied > 0 && rejected > 0) {
      ...
      console.log(
        pc.dim(`  ${filename}: `) + pc.yellow(`${rejected} rejected`),
      );
    }
  }
