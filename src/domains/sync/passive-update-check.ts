...
export async function maybeShowConfigUpdateNotification(
  ...
  try {
    ...
    if (detection.format === "multi-kit" && detection.metadata.kits) {
      for (const [kitType, kitInfo] of Object.entries(
        detection.metadata.kits,
      )) {
        if (kitInfo?.version) {
          kitsToCheck.push({
            kit: kitType as KitType,
    ...
    logger.debug(
      `Passive update check failed: ${error instanceof Error ? error.message : error}`,
    );
    return false;
  }
}
