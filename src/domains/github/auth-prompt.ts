...
export function showAuthOptions(): void {
  ...
  if (hasGit) {
    const sshStatus = hasSshKeys
      ? picocolors.green("✓ SSH keys detected")
      : "HTTPS mode";
    console.log(
      `  ${picocolors.cyan("--use-git")}     Use git clone (${sshStatus})`,
    );
  }

  console.log(
    `  ${picocolors.cyan("GITHUB_TOKEN")}  Set environment variable with Classic PAT`,
  );
  console.log(
    `  ${picocolors.cyan("gh auth login")} Install GitHub CLI and authenticate\n`,
  );
}
