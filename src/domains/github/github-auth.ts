...
export class AuthManager {
  ...
  private static tokenFetchPromise: Promise<{
    token: string;
    method: AuthMethod;
  }> | null = null;
   ...
   */
  private static async fetchToken(): Promise<{
    token: string;
    method: AuthMethod;
  }> {
...
Note: Do NOT use 'Paste an authentication token' - use web browser login.`,
  ...
  private static runDiagnostic(): string {
    ...
    try {
      ...
      if (tokenMatch)
        info.push(
          `  Token present: ${tokenMatch[1] !== "none" ? "yes" : "no"}`,
        );
    ...
    try {
      ...
      try {
        ...
        if (fallbackError?.stderr) {
          logger.debug(
            `gh auth token fallback stderr: ${fallbackError.stderr.toString().trim()}`,
          );
        }
      }
      return null;
