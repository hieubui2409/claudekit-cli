...
export function getDownloadableAsset(release: GitHubRelease): {
  ...
  if (release.assets.length === 0) {
    ...
    release.assets.forEach((asset, index) => {
      logger.debug(
        `  ${index + 1}. ${asset.name} (${(asset.size / 1024 / 1024).toFixed(2)} MB)`,
      );
    });
  }
  ...
  logger.debug(
    "No ClaudeKit package asset found, checking for other custom assets...",
  );
  ...
  logger.debug(
    "No custom assets found, falling back to GitHub automatic tarball",
  );
  return {
    type: "tarball",
