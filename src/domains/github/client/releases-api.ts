...
export class ReleasesApi {
  ...
  private applyPrereleasesFallback(
    ...
    if (processed.length === 0 && !includePrereleases) {
      ...
      if (withPrereleases.length > 0) {
        logger.warning(
          "No stable releases available. Showing prereleases instead.",
        );
      }
  ...
  async getLatestRelease(
    ...
    try {
      ...
      if (includePrereleases) {
        ...
        if (latestPrerelease) {
          logger.debug(
            `Found latest prerelease (by semver): ${latestPrerelease.tag_name}`,
          );
          return latestPrerelease;
        }
        logger.warning(
          "No prerelease versions found, falling back to latest stable release",
        );
      }
      ...
      if (latestStable) {
        logger.debug(
          `Found latest stable (by semver): ${latestStable.tag_name}`,
        );
        return latestStable;
      }
    ...
    try {
      ...
      while (allReleases.length < limit) {
        ...
        if (stopWhenStableFound) {
          ...
          if (hasStable) {
            logger.debug(
              `Found stable release on page ${page}, stopping pagination`,
            );
            break;
          }
        }
  ...
  async listReleasesWithCache(
    ...
    try {
      ...
      if (forceRefresh) {
        logger.debug(
          "Bypassing cache (--refresh flag) - fetching from GitHub API",
        );
      }
      if (!forceRefresh) {
      ...
      logger.error(
        `Failed to list releases with cache for ${kit.name}: ${error.message}`,
      );
      throw error;
    }
  }
