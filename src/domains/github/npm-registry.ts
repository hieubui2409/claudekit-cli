...
export class NpmRegistryClient {
  ...
  static async getPackageInfo(
    ...
    try {
      ...
      logger.debug(
        `Package info fetched: ${packageName}@${data["dist-tags"]?.latest}`,
      );
      return data;
    } catch (error) {
      if (error instanceof Error && error.name === "AbortError") {
        ...
        throw new Error(
          `Request timeout fetching ${packageName} from npm registry`,
        );
      }
      throw error;
    }
  ...
  static async getLatestVersion(
    ...
    try {
      const info = await NpmRegistryClient.getPackageInfo(
        packageName,
        registryUrl,
      );
      if (!info) return null;
      ...
      logger.debug(
        `Failed to get latest version for ${packageName}: ${message}`,
      );
      return null;
    }
  }
  ...
  static async versionExists(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error";
      logger.debug(
        `Failed to check version ${version} for ${packageName}: ${message}`,
      );
      return false;
    }
  }
  ...
  static async getVersionInfo(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error";
      logger.debug(
        `Failed to get version info for ${packageName}@${version}: ${message}`,
      );
      return null;
    }
  }
