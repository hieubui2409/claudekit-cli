...
export class ReleaseCache {
  ...
  private static readonly CACHE_TTL_SECONDS =
    Number(process.env.CK_CACHE_TTL) || 3600; // 1 hour (configurable via CK_CACHE_TTL env var)
  private readonly cacheDir: string;

  constructor() {
    this.cacheDir = join(
      PathResolver.getCacheDir(false),
      ReleaseCache.CACHE_DIR,
    );
  }
  ...
  async get(key: string): Promise<GitHubRelease[] | null> {
    ...
    try {
      ...
      logger.debug(
        `Release cache hit for key: ${key}, found ${releases.length} releases`,
      );
      return releases;
    } catch (error) {
  ...
  async set(key: string, releases: GitHubRelease[]): Promise<void> {
    ...
    try {
      ...
      logger.debug(
        `Release cache set for key: ${key}, cached ${releases.length} releases`,
      );
    } catch (error) {
  ...
  private isExpired(timestamp: number): boolean {
    ...
    logger.debug(
      `Cache age check: ${ageMinutes} minutes old, expired=${isExpired}`,
    );
