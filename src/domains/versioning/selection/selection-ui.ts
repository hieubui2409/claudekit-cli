...
export async function handleNoReleases(
  ...
  if (isNonInteractive()) {
    ...
    logger.info(
      "Provide a specific version with --version flag or run in interactive mode",
    );
    return null;
  }
...
export async function createVersionPrompt(
  ...
  if (isNonInteractive()) {
    const latestStable = releases.find(
      (r) => r.isLatestStable && !r.prerelease,
    );
    if (latestStable) {
      logger.info(
        `Non-interactive mode: selecting latest stable version ${latestStable.tag_name}`,
      );
    ...
    if (releases.length > 0) {
      logger.info(
        `Non-interactive mode: selecting version ${releases[0].tag_name}`,
      );
  ...
  for (const choice of versionChoices) {
    ...
    const isCurrentlyInstalled =
      currentVersion &&
      (choice.value === currentVersion ||
        choice.value === `v${currentVersion}`);
...
export async function handleSelectionError(
  ...
  if (isNonInteractive()) {
    logger.warning(
      "Non-interactive mode: version selection failed, cannot retry",
    );
    return null;
  }
