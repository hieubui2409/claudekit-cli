...
function renderCommandHeader(help: CommandHelp, theme: ColorTheme): string {
  const parts = [
    theme.command(help.name),
    theme.muted("-"),
    theme.description(help.description),
  ];
...
function renderOptionGroup(group: OptionGroup, theme: ColorTheme): string[] {
  ...
  const maxFlagWidth = Math.max(
    ...group.options.map((opt) => opt.flags.length),
  );
 ...
 */
function renderDeprecationWarning(
  help: CommandHelp,
  theme: ColorTheme,
): string {
...
export function renderGlobalHelp(
  ...
  lines.push("");
  lines.push(
    theme.muted("Run 'ck <command> --help' for detailed command information"),
  );
