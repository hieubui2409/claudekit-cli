...
export class AuthChecker implements Checker {
  ...
  private checkGitAvailable(): CheckResult {
    ...
    if (!gitInstalled) {
      return {
        ...
        suggestion:
          "Install git to use --use-git flag: https://git-scm.com/downloads",
        autoFixable: false,
      };
    }
  ...
  private async checkGhAuth(): Promise<CheckResult> {
    ...
    try {
      ...
      logger.verbose(
        "AuthChecker: Running 'gh auth status -h github.com' command",
      );
      ...
      return {
        ...
        suggestion:
          "Run: gh auth login -h github.com (select 'Login with a web browser')",
        autoFixable: true,
  ...
  private async checkRepoAccess(kit: KitType): Promise<CheckResult> {
    ...
    if (shouldSkipExpensiveOperations()) {
      logger.verbose(
        `AuthChecker: Skipping repo access check for ${kit} in CI/test`,
      );
      return {
        id: `repo-access-${kit}`,
    ...
    try {
      logger.verbose(
        `AuthChecker: Checking access to ${kitConfig.owner}/${kitConfig.repo}`,
      );
      const client = new GitHubClient();
      const hasAccess = await client.checkAccess(kitConfig);
      logger.verbose(`AuthChecker: Repo access check complete for ${kit}`, {
        hasAccess,
      });
      ...
      return {
        ...
        details: error instanceof Error ? error.message : "Unknown error",
        suggestion:
          "Re-authenticate: gh auth login (select 'Login with a web browser')",
        autoFixable: false,
      };
    }
  }
    ...
    return {
      ...
      execute: async (): Promise<FixResult> => {
        ...
        return {
          success: false,
          message:
            "Run manually: gh auth login -h github.com (select 'Login with a web browser')",
          details:
            "This command requires interactive input. Use web browser login, not PAT.",
        };
      },
    };
  }
