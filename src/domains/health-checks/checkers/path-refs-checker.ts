 ...
 */
export async function checkPathRefsValid(
  projectDir: string,
): Promise<CheckResult> {
  ...
  try {
    ...
    for (const ref of refs) {
      ...
      if (
        ref.startsWith("$HOME") ||
        ref.startsWith("${HOME}") ||
        ref.startsWith("%USERPROFILE%")
      ) {
        refPath = normalize(
          ref.replace(/^\$\{?HOME\}?/, home).replace("%USERPROFILE%", home),
        );
      }
      ...
      if (!isWithinHome && !isWithinBase && !isAbsoluteAllowed) {
        logger.verbose("Skipping potentially unsafe path reference", {
          ref,
          refPath,
        });
        continue;
