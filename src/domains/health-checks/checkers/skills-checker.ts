...
export function checkSkillsScripts(setup: ClaudeKitSetup): CheckResult[] {
  ...
  if (setup.global.path) {
    ...
    results.push({
      ...
      suggestion: !hasGlobalScript
        ? "Run: ck init --global --install-skills"
        : undefined,
      autoFixable: false,
    });
  }
  ...
  if (setup.project.metadata) {
    ...
    results.push({
      ...
      suggestion: !hasProjectScript
        ? "Run: ck init --install-skills"
        : undefined,
      autoFixable: false,
    });
  }
...
export function checkComponentCounts(setup: ClaudeKitSetup): CheckResult {
  ...
  const totalComponents =
    totalAgents + totalCommands + totalWorkflows + totalSkills;
  ...
  return {
    ...
    suggestion:
      totalComponents === 0
        ? "Install ClaudeKit: ck new --kit engineer"
        : undefined,
    autoFixable: false,
  };
}
