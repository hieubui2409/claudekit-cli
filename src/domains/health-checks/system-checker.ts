...
export class SystemChecker implements Checker {
  ...
  private async mapDependencyToCheck(
    ...
    return {
      ...
      suggestion: !isInstalled
        ? this.getSuggestion(dep.name as DependencyName)
        : undefined,
      autoFixable: this.isAutoFixable(dep.name as DependencyName),
      fix: !isInstalled
        ? this.createInstallFix(dep.name as DependencyName)
        : undefined,
    };
  }
    ...
    return {
      ...
      execute: async (): Promise<FixResult> => {
        ...
        return {
          ...
          details: result.installedVersion
