...
export class DoctorUIRenderer {
  ...
  renderHealingSummary(healSummary: HealingSummary): void {
    ...
    for (const fix of healSummary.fixes) {
      const symbol = fix.success
        ? pc.green(this.symbols.pass)
        : pc.red(this.symbols.fail);
      console.log(
        `│  ${symbol} ${pc.bold(fix.checkName)}  ${pc.dim(fix.message)}`,
      );
  ...
  private renderSummaryLine(summary: CheckSummary): void {
    ...
    console.log(
      `│  ${pc.dim("  ck init        Install/update ClaudeKit in project")}`,
    );
    console.log(
      `│  ${pc.dim("  ck init -g     Install/update ClaudeKit globally")}`,
    );
    console.log(`│  ${pc.dim("  ck update      Update the CLI tool")}`);
    console.log(
      `│  ${pc.dim("  ck uninstall   Remove ClaudeKit from project/global")}`,
    );
