...
export const ExcludePatternSchema = z
  ...
  .refine(
    (val) => !val.startsWith("/"),
    "Absolute paths not allowed in exclude patterns",
  )
  .refine(
    (val) => !val.includes(".."),
    "Path traversal not allowed in exclude patterns",
  );
...
export const UninstallCommandOptionsSchema = z
  ...
  .merge(GlobalOutputOptionsSchema);
export type UninstallCommandOptions = z.infer<
  typeof UninstallCommandOptionsSchema
>;
