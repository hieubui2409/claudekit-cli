...
export function registerCommands(cli: ReturnType<typeof cac>): void {
  ...
  cli
    ...
    .option(
      "--kit <kit>",
      "Kit to use: engineer, marketing, all, or comma-separated",
    )
    .option(
      "-r, --release <version>",
    ...
    .option(
      "--gemini",
      "Install Google Gemini CLI package (non-interactive mode)",
    )
    .option(
      "--install-skills",
      "Install skills dependencies (non-interactive mode)",
    )
    .option(
      "--with-sudo",
    ...
    .option(
      "--refresh",
      "Bypass release cache to fetch latest versions from GitHub",
    )
    .option("--docs-dir <name>", "Custom docs folder name (default: docs)")
    .option("--plans-dir <name>", "Custom plans folder name (default: plans)")
    .option(
      "-y, --yes",
      "Non-interactive mode with sensible defaults (skip all prompts)",
    )
    .option(
      "--use-git",
      "Use git clone instead of GitHub API (uses SSH/HTTPS credentials)",
    )
  ...
  cli
    ...
    .option(
      "-g, --global",
      "Use platform-specific user configuration directory",
    )
    .option(
      "--fresh",
    ...
    .option(
      "--dry-run",
      "Preview changes without applying them (requires --prefix)",
    )
    .option(
      "--force-overwrite",
    ...
    .option(
      "--sync",
      "Sync config files from upstream with interactive hunk-by-hunk merge",
    )
    .option(
      "--use-git",
      "Use git clone instead of GitHub API (uses SSH/HTTPS credentials)",
  ...
  cli
    .command(
      "easter-egg",
      "🥚 Roll for a random discount code (Code Hunt 2025)",
    )
    .action(async () => {
