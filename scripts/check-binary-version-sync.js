...
function main() {
  ...
  console.log(
    `🔍 Checking binary versions against package.json version: ${packageVersion}`,
  );
  ...
  for (const binary of binaries) {
    if (fs.existsSync(binary)) {
      ...
      if (binaryVersion !== packageVersion) {
        ...
        console.log(
          `❌ ${binary} version mismatch: ${binaryVersion} != ${packageVersion}`,
        );
      } else {
  ...
  if (!allSynced) {
    ...
    console.log(
      "   bun build src/index.ts --compile --outfile bin/ck-darwin-arm64",
    );
    console.log(
      "   bun build src/index.ts --compile --outfile bin/ck-darwin-x64",
    );
    console.log(
      "   bun build src/index.ts --compile --outfile bin/ck-linux-x64",
    );
    console.log(
      "   bun build src/index.ts --compile --outfile bin/ck-win32-x64.exe",
    );
    process.exit(1);
  }
