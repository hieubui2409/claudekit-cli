...
async function prepare(pluginConfig, context) {
  ...
  try {
    ...
    if (fs.existsSync("bin/ck-linux-x64")) {
      ...
      try {
        const output = execSync("./bin/ck-linux-x64 --version", {
          encoding: "utf8",
        });
        if (output.includes(version)) {
          ...
          logger.warn(
            `⚠️ Binary version mismatch. Expected: ${version}, Got: ${output.trim()}`,
          );
        }
      } catch (error) {
    ...
    if (failedPlatforms.length > 0) {
      throw new Error(
        `Binary build failed for platforms: ${failedPlatforms.join(", ")}`,
      );
    }
