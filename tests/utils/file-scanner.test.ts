...
describe("FileScanner", () => {
  ...
  describe("getFiles", () => {
    ...
    test("should skip Claude Code internal directories", async () => {
      ...
      await writeFile(
        join(destDir, "projects", "project1.json"),
        "project data",
      );
      await writeFile(
        join(destDir, "shell-snapshots", "snapshot.sh"),
        "shell history",
      );
      await writeFile(
        join(destDir, "file-history", "file1.json"),
        "file version",
      );
      ...
      await writeFile(
        join(destDir, ".anthropic", "config.json"),
        "claude config",
      );
      await writeFile(
        join(destDir, "telemetry", "data.json"),
        "telemetry data",
      );
      await writeFile(
        join(destDir, "claudekit-files", "my-file.txt"),
        "claudekit file",
      );
  ...
  describe("findCustomFiles", () => {
    ...
    test("should handle nested subdirectories", async () => {
      ...
      await writeFile(
        join(destNestedDir, "standard-cmd.md"),
        "standard command",
      );
      await writeFile(
        join(sourceNestedDir, "standard-cmd.md"),
        "standard command",
      );
    ...
    test("should handle files with special characters in names", async () => {
      ...
      await writeFile(
        join(destClaudeDir, "file_with_underscore.md"),
        "content",
      );
