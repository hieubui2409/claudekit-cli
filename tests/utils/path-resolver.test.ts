...
describe("PathResolver", () => {
  ...
  describe("isValidComponentName", () => {
    ...
    it("should reject path traversal attempts", () => {
      ...
      expect(PathResolver.isValidComponentName("../../../etc/passwd")).toBe(
        false,
      );
    });
    ...
    it("should reject absolute paths", () => {
      ...
      expect(PathResolver.isValidComponentName("C:\\Windows\\System32")).toBe(
        false,
      );
    });
    ...
    if (originalPlatform === "win32") {
      ...
      it("should use fallback if LOCALAPPDATA is not set on Windows", () => {
        ...
        expect(cacheDir).toBe(
          join(homedir(), "AppData", "Local", "claude", "cache"),
        );
      });
    } else {
  ...
  describe("buildSkillsPath", () => {
    ...
    it("should work with real home directory paths", () => {
      ...
      const globalPath = PathResolver.buildSkillsPath(
  ...
  describe("buildComponentPath", () => {
    ...
    it("should work with different component types", () => {
      ...
      components.forEach((component) => {
        const localPath = PathResolver.buildComponentPath(
          basePath,
          component,
          false,
        );
        const globalPath = PathResolver.buildComponentPath(
          basePath,
          component,
          true,
        );
    ...
    it("should reject empty or dangerous component names", () => {
      ...
      expect(() =>
        PathResolver.buildComponentPath(basePath, "", false),
      ...
      expect(() =>
        PathResolver.buildComponentPath(basePath, "../etc", false),
      ).toThrow("Invalid component name");
      expect(() =>
        PathResolver.buildComponentPath(basePath, "foo/../bar", true),
      ).toThrow("Invalid component name");
    });
    ...
    it("should allow valid component names with special characters", () => {
      ...
      const specialLocal = PathResolver.buildComponentPath(
        basePath,
        "test-dir",
        false,
      );
      const specialGlobal = PathResolver.buildComponentPath(
        basePath,
        "test_dir",
        true,
      );
  ...
  describe("path consistency", () => {
    ...
    it("should maintain consistency between new and existing methods", () => {
      ...
      const localSkillsManual = join(
        baseDir,
        PathResolver.getPathPrefix(false),
        "skills",
      );
      const localSkillsMethod = PathResolver.buildSkillsPath(baseDir, false);
      expect(localSkillsManual).toBe(localSkillsMethod);

      const globalSkillsManual = join(
        baseDir,
        PathResolver.getPathPrefix(true),
        "skills",
      );
      ...
      const localComponentManual = join(
        baseDir,
        PathResolver.getPathPrefix(false),
        component,
      );
      const localComponentMethod = PathResolver.buildComponentPath(
        baseDir,
        component,
        false,
      );
      ...
      const globalComponentManual = join(
        baseDir,
        PathResolver.getPathPrefix(true),
        component,
      );
      const globalComponentMethod = PathResolver.buildComponentPath(
        baseDir,
        component,
        true,
      );
