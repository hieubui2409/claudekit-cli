  ...
  describe("cleanupCommandsDirectory", () => {
    ...
    test("preserves CK-modified files", async () => {
      ...
      const originalChecksum =
        await OwnershipChecker.calculateChecksum(modifiedFile);
    ...
    test("skips cleanup gracefully on legacy install (no metadata files[])", async () => {
      ...
      const result = await CommandsPrefix.cleanupCommandsDirectory(
        tempDir,
        false,
      );
    ...
    test("handles nested directories correctly", async () => {
      ...
      const nestedChecksum =
        await OwnershipChecker.calculateChecksum(nestedCkFile);
