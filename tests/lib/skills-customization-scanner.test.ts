...
describe("SkillsCustomizationScanner", () => {
  ...
  describe("scanCustomizations - without baseline", () => {
    test("should return non-customized for all skills when no baseline", async () => {
      ...
      const customizations =
        await SkillsCustomizationScanner.scanCustomizations(currentSkillsDir);
    ...
    test("should skip hidden directories and node_modules", async () => {
      ...
      await writeFile(
        join(currentSkillsDir, "node_modules", "file.txt"),
        "node",
      );
      await mkdir(join(currentSkillsDir, "valid-skill"));
      await writeFile(
        join(currentSkillsDir, "valid-skill", "skill.md"),
        "valid",
      );
  ...
  describe("scanCustomizations - with baseline directory", () => {
    test("should detect unchanged skills", async () => {
      ...
      await mkdir(join(currentSkillsDir, "skill1"));
      await writeFile(
        join(currentSkillsDir, "skill1", "skill.md"),
        "identical content",
      );

      await mkdir(join(baselineSkillsDir, "skill1"));
      await writeFile(
        join(baselineSkillsDir, "skill1", "skill.md"),
        "identical content",
      );

      const customizations =
        await SkillsCustomizationScanner.scanCustomizations(
          currentSkillsDir,
          baselineSkillsDir,
        );
    ...
    test("should detect added files in skill", async () => {
      ...
      await writeFile(join(currentSkillsDir, "skill1", "skill.md"), "content");
      await writeFile(
        join(currentSkillsDir, "skill1", "extra.md"),
        "extra file",
      );
      ...
      const addedFile = customizations[0].changes?.find(
        (c) => c.type === "added",
      );
    ...
    test("should detect deleted files in skill", async () => {
      ...
      await writeFile(join(baselineSkillsDir, "skill1", "skill.md"), "content");
      await writeFile(
        join(baselineSkillsDir, "skill1", "removed.md"),
        "removed file",
      );
      ...
      const deletedFile = customizations[0].changes?.find(
        (c) => c.type === "deleted",
      );
    ...
    test("should detect custom skills not in baseline", async () => {
      ...
      await writeFile(
        join(currentSkillsDir, "custom-skill", "skill.md"),
        "custom",
      );

      await mkdir(join(baselineSkillsDir, "standard-skill"));
      await writeFile(
        join(baselineSkillsDir, "standard-skill", "skill.md"),
        "standard",
      );
    ...
    test("should handle nested file structures", async () => {
      ...
      await mkdir(join(currentSkillsDir, "skill1", "subdir"), {
        recursive: true,
      });
      await writeFile(join(currentSkillsDir, "skill1", "skill.md"), "main");
      await writeFile(
        join(currentSkillsDir, "skill1", "subdir", "nested.md"),
        "nested modified",
      );

      // Baseline has same structure with different nested content
      await mkdir(join(baselineSkillsDir, "skill1", "subdir"), {
        recursive: true,
      });
      await writeFile(join(baselineSkillsDir, "skill1", "skill.md"), "main");
      await writeFile(
        join(baselineSkillsDir, "skill1", "subdir", "nested.md"),
        "nested original",
      );
    ...
    test("should handle multiple customized skills", async () => {
      ...
      await mkdir(join(currentSkillsDir, "skill2"));
      await writeFile(
        join(currentSkillsDir, "skill2", "skill.md"),
        "unchanged",
      );
      await mkdir(join(currentSkillsDir, "skill3"));
      await writeFile(
        join(currentSkillsDir, "skill3", "skill.md"),
        "also modified",
      );
      ...
      await writeFile(
        join(baselineSkillsDir, "skill2", "skill.md"),
        "unchanged",
      );
      await mkdir(join(baselineSkillsDir, "skill3"));
      await writeFile(
        join(baselineSkillsDir, "skill3", "skill.md"),
        "different",
      );
  ...
  describe("scanCustomizations - categorized structure", () => {
    test("should handle categorized directory structure", async () => {
      ...
      await mkdir(join(currentSkillsDir, "category1", "skill1"), {
        recursive: true,
      });
      await writeFile(
        join(currentSkillsDir, "category1", "skill1", "skill.md"),
        "modified",
      );

      // Create categorized baseline structure
      await mkdir(join(baselineSkillsDir, "category1", "skill1"), {
        recursive: true,
      });
      await writeFile(
        join(baselineSkillsDir, "category1", "skill1", "skill.md"),
        "original",
      );
    ...
    test("should handle multiple categories", async () => {
      ...
      await mkdir(join(currentSkillsDir, "cat1", "skill1"), {
        recursive: true,
      });
      await writeFile(
        join(currentSkillsDir, "cat1", "skill1", "skill.md"),
        "content1",
      );
      await mkdir(join(currentSkillsDir, "cat2", "skill2"), {
        recursive: true,
      });
      await writeFile(
        join(currentSkillsDir, "cat2", "skill2", "skill.md"),
        "content2",
      );

      await mkdir(join(baselineSkillsDir, "cat1", "skill1"), {
        recursive: true,
      });
      await writeFile(
        join(baselineSkillsDir, "cat1", "skill1", "skill.md"),
        "content1",
      );
      await mkdir(join(baselineSkillsDir, "cat2", "skill2"), {
        recursive: true,
      });
      await writeFile(
        join(baselineSkillsDir, "cat2", "skill2", "skill.md"),
        "different",
      );
  ...
  describe("scanCustomizations - edge cases", () => {
    ...
    test("should handle skills with special characters in filenames", async () => {
      await mkdir(join(currentSkillsDir, "skill1"));
      await writeFile(
        join(currentSkillsDir, "skill1", "file-with-dash.md"),
        "content",
      );
      await writeFile(
        join(currentSkillsDir, "skill1", "file_with_underscore.md"),
        "content",
      );

      await mkdir(join(baselineSkillsDir, "skill1"));
      await writeFile(
        join(baselineSkillsDir, "skill1", "file-with-dash.md"),
        "content",
      );
    ...
    test("should handle large files", async () => {
      ...
      await mkdir(join(currentSkillsDir, "skill1"));
      await writeFile(
        join(currentSkillsDir, "skill1", "large.md"),
        largeContent,
      );
  ...
  describe("scanCustomizations - file change details", () => {
    test("should include hash information in changes", async () => {
      ...
      const modifiedChange = customizations[0].changes?.find(
        (c) => c.type === "modified",
      );
    ...
    test("should include all change types in one skill", async () => {
      ...
      await writeFile(
        join(baselineSkillsDir, "skill1", "deleted.md"),
        "deleted",
      );
      // added.md is missing
