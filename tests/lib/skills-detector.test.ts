...
describe("SkillsMigrationDetector", () => {
  ...
  describe("detectMigration - basic scenarios", () => {
    test("should return not_needed when empty directories exist", async () => {
      ...
      const result = await SkillsMigrationDetector.detectMigration(
        oldSkillsDir,
        currentSkillsDir,
      );
    ...
    test("should return not_needed for same categorized structure", async () => {
      ...
      await mkdir(join(oldSkillsDir, "category1", "skill1"), {
        recursive: true,
      });
      await writeFile(
        join(oldSkillsDir, "category1", "skill1", "skill.md"),
        "skill 1",
      );
  ...
  describe("detectMigration - migration needed scenarios", () => {
    test("should detect migration from flat to categorized", async () => {
      ...
      // New: categorized
      await mkdir(join(oldSkillsDir, "ai-multimodal", "gemini-vision"), {
        recursive: true,
      });
      await writeFile(
    ...
    test("should generate skill mappings for migratable skills", async () => {
      ...
      await writeFile(
        join(oldSkillsDir, "devops", "docker", "skill.md"),
        "docker",
      );
      ...
      const visionMapping = result.skillMappings.find(
        (m) => m.skillName === "gemini-vision",
      );
      expect(visionMapping).toBeDefined();
      expect(visionMapping?.category).toBe("ai-multimodal");

      const dockerMapping = result.skillMappings.find(
        (m) => m.skillName === "docker",
      );
  ...
  describe("detectMigration - manifest-based detection", () => {
    test("should use manifest for detection when available", async () => {
      ...
      await SkillsManifestManager.writeManifest(
        currentSkillsDir,
        currentManifest,
      );

      const newManifest =
        await SkillsManifestManager.generateManifest(oldSkillsDir);
  ...
  describe("detectMigration - skill mapping generation", () => {
    ...
    test("should handle multiple skills in different categories", async () => {
      ...
      await mkdir(join(oldSkillsDir, "web-frameworks", "nextjs"), {
        recursive: true,
      });
      await writeFile(
        join(oldSkillsDir, "web-frameworks", "nextjs", "skill.md"),
        "nextjs",
      );
    ...
    test("should not generate mappings for unknown skills", async () => {
      ...
      await mkdir(join(oldSkillsDir, "category", "unknown-skill"), {
        recursive: true,
      });
      await writeFile(
        join(oldSkillsDir, "category", "unknown-skill", "skill.md"),
        "unknown",
      );
  ...
  describe("detectMigration - edge cases", () => {
    ...
    test("should handle skills with multiple files", async () => {
      ...
      await writeFile(
        join(currentSkillsDir, "gemini-vision", "helper.js"),
        "code",
      );
