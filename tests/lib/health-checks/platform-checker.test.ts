...
describe("PlatformChecker", () => {
  ...
  describe("run", () => {
    test("executes all checks on current platform", async () => {
      const { PlatformChecker } =
        await import("../../../src/domains/health-checks/platform-checker.js");
  ...
  describe("checkPlatformDetect", () => {
    test("returns valid platform information", async () => {
      const { checkPlatformDetect } =
        await import("../../../src/domains/health-checks/platform/environment-checker.js");
      ...
      expect(result.message).toMatch(
        /^(linux|darwin|win32) \([a-z0-9_]+\)( - WSL: .*)?$/,
      );
    });
  });
  ...
  describe("checkShellDetection", () => {
    test("detects shell from environment", async () => {
      const { checkShellDetection } =
        await import("../../../src/domains/health-checks/platform/shell-checker.js");
