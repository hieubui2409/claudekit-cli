...
describe("ClaudeKitChecker - Enhanced Checks", () => {
  ...
  describe("checkGlobalDirReadable", () => {
    test("passes when metadata.json exists and is readable", async () => {
      ...
      const { checkGlobalDirReadable } =
        await import("../../../src/domains/health-checks/checkers/permissions-checker.js");
  ...
  describe("checkGlobalDirWritable", () => {
    ...
    test("fails when directory is not writable", async () => {
      ...
      spyOn(mockFs, "writeFile").mockRejectedValue(
        new Error("EACCES: permission denied"),
      );
  ...
  describe("checkHooksExist", () => {
    test("returns info when no hooks directory exists", async () => {
      const { checkHooksExist } =
        await import("../../../src/domains/health-checks/checkers/hooks-checker.js");
    ...
    test("counts hooks in project directory", async () => {
      ...
      await writeFile(
        join(projectHooksDir, "project-hook.js"),
        "console.log('project');",
      );
    ...
    test("counts hooks from both directories", async () => {
      ...
      mkdirSync(projectHooksDir, { recursive: true });
      await writeFile(
        join(globalHooksDir, "global.js"),
        "console.log('global');",
      );
      await writeFile(
        join(projectHooksDir, "project.js"),
        "console.log('project');",
      );
  ...
  describe("checkSettingsValid", () => {
    test("passes with valid global settings.json", async () => {
      ...
      const { checkSettingsValid } =
        await import("../../../src/domains/health-checks/checkers/settings-checker.js");
  ...
  describe("checkPathRefsValid", () => {
    test("returns info when no CLAUDE.md exists", async () => {
      const { checkPathRefsValid } =
        await import("../../../src/domains/health-checks/checkers/path-refs-checker.js");
    ...
    test("warns about broken @path references", async () => {
      ...
      expect(result.suggestion).toBe(
        "Some @path references point to missing files",
      );
    });
    ...
    test("prefers global CLAUDE.md over project", async () => {
      ...
      mkdirSync(join(mockProjectDir, "claude-project", ".claude"), {
        recursive: true,
      });
      ...
      const { checkPathRefsValid } =
        await import("../../../src/domains/health-checks/checkers/path-refs-checker.js");
      const result = await checkPathRefsValid(
        join(mockProjectDir, "claude-project"),
      );
  ...
  describe("checkProjectConfigCompleteness", () => {
    test("returns info when not in a project directory", async () => {
      ...
      const { checkProjectConfigCompleteness } =
        await import("../../../src/domains/health-checks/checkers/config-completeness-checker.js");
    ...
    test("fails when only CLAUDE.md exists", async () => {
      ...
      expect(result.suggestion).toBe(
        "Run 'ck init' to install complete ClaudeKit in project",
      );
    });
    ...
    test("warns when some directories are missing", async () => {
      ...
      expect(result.suggestion).toBe(
        "Run 'ck init' to update project configuration",
      );
    });
  ...
  describe("Edge Cases", () => {
    test("handles special characters in paths", async () => {
      ...
      mkdirSync(globalHooksDir, { recursive: true });
      await writeFile(
        join(globalHooksDir, "special-chars@#$%.js"),
        "console.log('special');",
      );
