...
describe("SkillsBackupManager", () => {
  ...
  describe("createBackup", () => {
    test("should create backup of skills directory", async () => {
      ...
      expect(await pathExists(join(backupPath, "skill1", "skill.md"))).toBe(
        true,
      );
      expect(await pathExists(join(backupPath, "skill2", "skill.md"))).toBe(
        true,
      );

      // Verify content
      const content1 = await Bun.file(
        join(backupPath, "skill1", "skill.md"),
      ).text();
    ...
    test("should create backup in specified parent directory", async () => {
      ...
      const backupPath = await SkillsBackupManager.createBackup(
        skillsDir,
        customParent,
      );
    ...
    test("should throw error for non-existent skills directory", async () => {
      await expect(
        SkillsBackupManager.createBackup(join(testDir, "nonexistent")),
    ...
    test("should handle nested directory structures", async () => {
      await mkdir(join(skillsDir, "skill1", "subdir", "deep"), {
        recursive: true,
      });
      await writeFile(join(skillsDir, "skill1", "skill.md"), "main");
      await writeFile(
        join(skillsDir, "skill1", "subdir", "nested.md"),
        "nested",
      );
      await writeFile(
        join(skillsDir, "skill1", "subdir", "deep", "deep.md"),
        "deep",
      );
      ...
      expect(
        await pathExists(join(backupPath, "skill1", "subdir", "nested.md")),
      ).toBe(true);
      expect(
        await pathExists(
          join(backupPath, "skill1", "subdir", "deep", "deep.md"),
        ),
      ).toBe(true);
    });
  ...
  describe("restoreBackup", () => {
    test("should restore skills from backup", async () => {
      ...
      const content = await Bun.file(
        join(skillsDir, "skill1", "skill.md"),
      ).text();
    ...
    test("should throw error for non-existent backup", async () => {
      await expect(
        SkillsBackupManager.restoreBackup(
          join(testDir, "nonexistent-backup"),
          skillsDir,
        ),
      ).rejects.toThrow(SkillsMigrationError);
    });
    ...
    test("should restore nested directory structures", async () => {
      ...
      expect(
        await pathExists(join(skillsDir, "skill1", "subdir", "nested.md")),
      ).toBe(true);
    });
  });
  ...
  describe("listBackups", () => {
    test("should list all backup directories", async () => {
      ...
      const backup1 = await SkillsBackupManager.createBackup(
        skillsDir,
        parentDir,
      );
      ...
      const backup2 = await SkillsBackupManager.createBackup(
        skillsDir,
        parentDir,
      );
    ...
    test("should sort backups by timestamp (newest first)", async () => {
      ...
      const backup3 = await SkillsBackupManager.createBackup(
        skillsDir,
        parentDir,
      );
    ...
    test("should return empty array for non-existent directory", async () => {
      const backups = await SkillsBackupManager.listBackups(
  ...
  describe("getBackupSize", () => {
    ...
    test("should return 0 for non-existent backup", async () => {
      const size = await SkillsBackupManager.getBackupSize(
        join(testDir, "nonexistent"),
      );
  ...
  describe("extractBackupTimestamp", () => {
    ...
    test("should return null for non-backup directory", () => {
      const timestamp =
        SkillsBackupManager.extractBackupTimestamp("/some/random/path");
  ...
  describe("integration tests", () => {
    test("should backup and restore successfully", async () => {
      ...
      await writeFile(
        join(skillsDir, "different-skill", "skill.md"),
        "different",
      );
