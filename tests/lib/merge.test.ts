...
describe("FileMerger", () => {
  ...
  describe("merge with skipConfirmation", () => {
    ...
    test("should skip protected patterns like *.key if they exist in destination", async () => {
      ...
      const keyContent = await Bun.file(
        join(testDestDir, "private.key"),
      ).text();
    ...
    test("should handle nested directories", async () => {
      ...
      expect(existsSync(join(testDestDir, "nested", "deep", "file.txt"))).toBe(
        true,
      );
    });
  ...
  describe("two-tier protection system", () => {
    describe("Tier 1: Security-sensitive files (NEVER_COPY_PATTERNS)", () => {
      ...
      test("should NEVER copy .env.*.local files", async () => {
        await writeFile(join(testSourceDir, "normal.txt"), "normal");
        await writeFile(
          join(testSourceDir, ".env.production.local"),
          "SECRET=prod",
        );
        ...
        expect(existsSync(join(testDestDir, "normal.txt"))).toBe(true);
        expect(existsSync(join(testDestDir, ".env.production.local"))).toBe(
          false,
        );
      });
    ...
    describe("Tier 2: User config files (USER_CONFIG_PATTERNS)", () => {
      ...
      test("should copy .repomixignore on first install", async () => {
        ...
        const content = await Bun.file(
          join(testDestDir, ".repomixignore"),
        ).text();
      ...
      test("should preserve all existing user config files on update", async () => {
        ...
        expect(await Bun.file(join(testDestDir, ".mcp.json")).text()).toBe(
          '{"custom": true}',
        );
        expect(await Bun.file(join(testDestDir, "CLAUDE.md")).text()).toBe(
          "# Custom CLAUDE.md",
        );
        expect(existsSync(join(testDestDir, "normal.txt"))).toBe(true);
      });
    ...
    describe("Conflict detection with two-tier system", () => {
      ...
      test("should report normal file overwrites as conflicts", async () => {
        ...
        expect(await Bun.file(join(testDestDir, "file1.txt")).text()).toBe(
          "new content 1",
        );
        expect(await Bun.file(join(testDestDir, "file2.txt")).text()).toBe(
          "new content 2",
        );
      });
    });
  ...
  describe("edge cases", () => {
    ...
    test("should skip custom ignore patterns if they exist in destination", async () => {
      ...
      const customContent = await Bun.file(
        join(testDestDir, "custom-ignore.txt"),
      ).text();
  ...
  describe("include patterns", () => {
    test("should only copy files matching include patterns", async () => {
      ...
      expect(
        existsSync(join(testDestDir, ".claude", "agents", "researcher.md")),
      ).toBe(true);
      expect(
        existsSync(join(testDestDir, ".claude", "hooks", "pre-commit")),
      ).toBe(true);

      // Should NOT copy commands
      expect(
        existsSync(join(testDestDir, ".claude", "commands", "test.md")),
      ).toBe(false);
    });

    test("should support nested directory patterns", async () => {
      // Create nested structure
      const researcherDir = join(
        testSourceDir,
        ".claude",
        "agents",
        "researcher",
      ...
      expect(
        existsSync(
          join(testDestDir, ".claude", "agents", "researcher", "config.md"),
        ),
      ).toBe(true);
      ...
      expect(
        existsSync(
          join(testDestDir, ".claude", "agents", "planner", "config.md"),
        ),
      ).toBe(false);
    });
    ...
    test("should handle multiple specific subdirectories", async () => {
      ...
      expect(
        existsSync(
          join(testDestDir, ".claude", "agents", "researcher", "file.md"),
        ),
      ).toBe(true);
      expect(
        existsSync(
          join(testDestDir, ".claude", "agents", "planner", "file.md"),
        ),
      ).toBe(true);

      // Should copy hooks
      expect(existsSync(join(testDestDir, ".claude", "hooks", "hook.sh"))).toBe(
        true,
      );

      // Should NOT copy unselected tester
      expect(
        existsSync(join(testDestDir, ".claude", "agents", "tester", "file.md")),
      ).toBe(false);
    });
  });
  ...
  describe("custom .claude file preservation", () => {
    test("should preserve custom .claude files when patterns are added", async () => {
      ...
      await writeFile(
        join(destClaudeDir, "standard.md"),
      ...
      const standardContent = await Bun.file(
        join(destClaudeDir, "standard.md"),
      ).text();
    ...
    test("should preserve nested custom .claude files", async () => {
      ...
      await writeFile(
        join(sourceCommandsDir, "standard-cmd.md"),
        "standard command",
      );
      ...
      const customContent = await Bun.file(
        join(destCommandsDir, "custom-cmd.md"),
      ).text();
  ...
  describe("security: symlink handling", () => {
    test("should skip symbolic links during merge", async () => {
      ...
      await symlink(
        join(testSourceDir, "target.txt"),
        join(sourceDir, "symlink.txt"),
      );

      await merger.merge(testSourceDir, testDestDir, false);

      // Real file should be copied
      expect(existsSync(join(testDestDir, "files", "real-file.txt"))).toBe(
        true,
      );
    ...
    test("should skip directory symlinks during merge", async () => {
      ...
      expect(
        existsSync(join(testDestDir, "symlink-dir", "sensitive.txt")),
      ).toBe(false);
    });
  });
  ...
  describe("global mode: settings.json variable replacement", () => {
    test("should replace $CLAUDE_PROJECT_DIR with $HOME on Unix/Linux/Mac when isGlobal is true", async () => {
      ...
      try {
        ...
        const destContent = await Bun.file(
          join(testDestDir, "settings.json"),
        ).text();
    ...
    test("should replace $CLAUDE_PROJECT_DIR with %USERPROFILE% on Windows when isGlobal is true", async () => {
      ...
      try {
        ...
        expect(destJson["claude.skillsDir"]).toBe(
    ...
    test("should NOT replace $CLAUDE_PROJECT_DIR when isGlobal is false (Unix)", async () => {
      ...
      try {
        ...
        expect(destJson["claude.projectDir"]).toBe(
    ...
    test("should only process settings.json, not other JSON files", async () => {
      ...
      try {
        ...
        const settingsDestContent = await Bun.file(
          join(testDestDir, "settings.json"),
        ).text();
        ...
        const packageDestContent = await Bun.file(
          join(testDestDir, "package.json"),
        ).text();
    ...
    test("should handle settings.json in nested directories (not processed)", async () => {
      ...
      expect(existsSync(join(testDestDir, "nested", "settings.json"))).toBe(
        true,
      );
      const destContent = await Bun.file(
        join(testDestDir, "nested", "settings.json"),
      ).text();
  ...
  describe("file tracking for manifest", () => {
    ...
    test("should return top-level items with getInstalledItems()", async () => {
      ...
      await mkdir(join(testSourceDir, "agents", "researcher"), {
        recursive: true,
      });
      ...
      await writeFile(
        join(testSourceDir, "agents", "researcher", "config.md"),
        "config",
      );
      await writeFile(join(testSourceDir, "README.md"), "readme");
  ...
  describe("local mode: monorepo path transformation", () => {
    ...
    test("should transform to %USERPROFILE% in global mode (Windows)", async () => {
      ...
      try {
        ...
        expect(destJson.statusLine.command).toBe(
          'node "%USERPROFILE%"/.claude/statusline.cjs',
        );
      } finally {
        Object.defineProperty(process, "platform", {
          value: originalPlatform,
    ...
    test("should generate commands that work with paths containing spaces", async () => {
      ...
      await writeFile(
        testScript,
        'console.log("SUCCESS: Path with spaces works!");',
      );
    ...
    test("should generate commands that work with paths containing spaces on Windows", async () => {
      ...
      await writeFile(
        testScript,
        'console.log("SUCCESS: Windows path with spaces works!");',
      );
      ...
      try {
        ...
        const command = destJson.hooks.UserPromptSubmit[0].hooks[0].command;
        expect(command).toBe(
          'node "%USERPROFILE%"/.claude/hooks/test-hook.cjs',
        );
    ...
    test("should generate commands that work with paths containing special characters", async () => {
      ...
      const testHomeWithSpecialChars = join(
        tmpdir(),
        `test user (work) dir ${Date.now()}`,
      );
      ...
      await writeFile(
        testScript,
        'console.log("SUCCESS: Path with special chars works!");',
      );
