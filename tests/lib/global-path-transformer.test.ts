  ...
  describe("transformContent", () => {
    ...
    it("transforms ./.claude/ paths", () => {
      ...
      expect(transformed).toBe(
        `command: "node ${expectedPrefix}/.claude/hooks/test.js"`,
      );
    ...
    it("transforms @./.claude/ paths (@ with relative)", () => {
      ...
      expect(transformed).toBe(
        `workflow: @${expectedPrefix}/.claude/workflows/main.md`,
      );
      expect(changes).toBe(1);
    });
    ...
    it("transforms @.claude/ paths", () => {
      ...
      expect(transformed).toBe(
        `reference: @${expectedPrefix}/.claude/agents/planner.md`,
      );
      expect(changes).toBe(1);
    });
    ...
    it("transforms double-quoted .claude/ paths", () => {
      ...
      expect(transformed).toBe(
        `"node ${expectedPrefix}/.claude/hooks/dev-rules-reminder.js"`,
      );
      expect(changes).toBe(1);
    });
    ...
    it("transforms single-quoted .claude/ paths", () => {
      ...
      expect(transformed).toBe(
        `'node ${expectedPrefix}/.claude/hooks/test.js'`,
      );
      expect(changes).toBe(1);
    });
    ...
    it("transforms backtick-quoted .claude/ paths", () => {
      ...
      expect(transformed).toBe(
        `\`node ${expectedPrefix}/.claude/scripts/run.js\``,
      );
      expect(changes).toBe(1);
    });
    ...
    it("transforms parentheses (.claude/ paths (markdown links)", () => {
      ...
      expect(transformed).toBe(
        `See [workflows](${expectedPrefix}/.claude/workflows/main.md)`,
      );
      expect(changes).toBe(1);
    });
    ...
    it("transforms space-prefixed .claude/ paths", () => {
      ...
      expect(transformed).toBe(
        `Run the command: node ${expectedPrefix}/.claude/hooks/test.js`,
      );
      expect(changes).toBe(1);
    });
    ...
    it("transforms start-of-line .claude/ paths", () => {
      ...
      expect(transformed).toBe(
        `${expectedPrefix}/.claude/hooks/test.js is the file`,
      );
      expect(changes).toBe(1);
    });
  ...
  "hooks": {
    ...
    it("handles real-world settings.json content", () => {
      ...
      expect(transformed).toContain(
        `${expectedPrefix}/.claude/hooks/dev-rules-reminder.js`,
      );
      expect(transformed).toContain(
        `${expectedPrefix}/.claude/hooks/scout-block.js`,
      );
      expect(changes).toBe(2);
    });
  });
  ...
  describe("transformPathsForGlobalInstall", () => {
    ...
    it("transforms all eligible files in directory", async () => {
      ...
      expect(claudeContent).toContain(
        `@${HOME_PREFIX}/.claude/workflows/main.md`,
      );
    });
    ...
    it("only transforms files with eligible extensions", async () => {
      ...
      await writeFile(
        join(testDir, ".claude", "test.json"),
        '"path": ".claude/test"',
      );
      await writeFile(
        join(testDir, ".claude", "test.txt"),
        "path: .claude/test",
      ); // Not eligible
      ...
      const txtContent = await readFile(
        join(testDir, ".claude", "test.txt"),
        "utf-8",
      );
