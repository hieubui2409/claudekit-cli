...
describe("NpmRegistryClient", () => {
  ...
  describe("getPackageInfo", () => {
    ...
    test("returns null for 404 response", async () => {
      ...
      const result = await NpmRegistryClient.getPackageInfo(
        "nonexistent-package",
      );
    ...
    test("throws error for non-404 errors", async () => {
      ...
      await expect(
        NpmRegistryClient.getPackageInfo("test-package"),
    ...
    test("uses custom registry URL when provided", async () => {
      ...
      await NpmRegistryClient.getPackageInfo(
        "test-package",
  ...
  describe("versionExists", () => {
    test("returns true when version exists", async () => {
      ...
      const result = await NpmRegistryClient.versionExists(
        "test-package",
        "1.1.0",
      );
  ...
  describe("getVersionInfo", () => {
    test("returns version info when found", async () => {
      ...
      const result = await NpmRegistryClient.getVersionInfo(
        "test-package",
        "1.0.0",
      );
      expect(result).not.toBeNull();
