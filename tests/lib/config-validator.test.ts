...
describe("config-validator", () => {
  describe("GEMINI_API_KEY validation", () => {
    test("should accept valid Gemini API key", () => {
      ...
      expect(validateApiKey(validKey, VALIDATION_PATTERNS.GEMINI_API_KEY)).toBe(
        true,
      );
    });
    ...
    test("should reject invalid Gemini API key", () => {
      expect(
        validateApiKey("invalid", VALIDATION_PATTERNS.GEMINI_API_KEY),
      ).toBe(false);
      expect(validateApiKey("", VALIDATION_PATTERNS.GEMINI_API_KEY)).toBe(
        false,
      );
    });
  });
  ...
  describe("DISCORD_WEBHOOK_URL validation", () => {
    test("should accept valid Discord webhook", () => {
      ...
      expect(
        validateApiKey(validUrl, VALIDATION_PATTERNS.DISCORD_WEBHOOK_URL),
      ).toBe(true);
    });
    ...
    test("should reject invalid Discord webhook", () => {
      expect(
        validateApiKey(
          ...
          VALIDATION_PATTERNS.DISCORD_WEBHOOK_URL,
        ),
      ).toBe(false);
    });
  });
  ...
  describe("TELEGRAM_BOT_TOKEN validation", () => {
    test("should accept valid Telegram token", () => {
      ...
      expect(
        validateApiKey(validToken, VALIDATION_PATTERNS.TELEGRAM_BOT_TOKEN),
      ).toBe(true);
    });
    ...
    test("should reject invalid Telegram tokens", () => {
      ...
      // Empty string
      expect(validateApiKey("", VALIDATION_PATTERNS.TELEGRAM_BOT_TOKEN)).toBe(
        false,
      );
