...
describe("SkillsMigrator", () => {
  ...
  describe("migrate - no migration needed", () => {
    test("should succeed when no migration needed (same structure)", async () => {
      ...
      const result = await SkillsMigrator.migrate(
        newSkillsDir,
        currentSkillsDir,
        options,
      );
  ...
  describe("migrate - dry run mode", () => {
    test("should not make changes in dry run mode", async () => {
      ...
      // New: categorized
      await mkdir(join(newSkillsDir, "ai-multimodal", "gemini-vision"), {
        recursive: true,
      });
      await writeFile(
      ...
      expect(await pathExists(join(currentSkillsDir, "gemini-vision"))).toBe(
        true,
      );
      expect(await pathExists(join(currentSkillsDir, "ai-multimodal"))).toBe(
        false,
      );
    });
  });
  ...
  describe("migrate - non-interactive mode", () => {
    test("should migrate from flat to categorized structure", async () => {
      ...
      await writeFile(
        join(newSkillsDir, "devops", "docker", "skill.md"),
        "docker",
      );
      ...
      expect(
        await pathExists(
          join(currentSkillsDir, "ai-multimodal", "gemini-vision"),
        ),
      ).toBe(true);
      expect(await pathExists(join(currentSkillsDir, "devops", "docker"))).toBe(
        true,
      );

      // Verify old flat structure removed
      expect(
        await pathExists(join(currentSkillsDir, "gemini-vision", "skill.md")),
      ).toBe(false);
      expect(
        await pathExists(join(currentSkillsDir, "docker", "skill.md")),
      ).toBe(false);
    });
    ...
    test("should handle nested file structures in skills", async () => {
      await mkdir(join(currentSkillsDir, "gemini-vision", "subdir"), {
        recursive: true,
      });
      await writeFile(
        join(currentSkillsDir, "gemini-vision", "skill.md"),
      ...
      await writeFile(
        join(currentSkillsDir, "gemini-vision", "subdir", "nested.md"),
        "nested",
      );
    ...
    test("should skip migration for unknown skills", async () => {
      ...
      await writeFile(
        join(currentSkillsDir, "unknown-skill", "skill.md"),
        "unknown",
      );
      await mkdir(join(currentSkillsDir, "gemini-vision"));
      await writeFile(
      ...
      await mkdir(join(newSkillsDir, "category", "unknown-skill"), {
        recursive: true,
      });
      await writeFile(
        join(newSkillsDir, "category", "unknown-skill", "skill.md"),
        "unknown",
      );
    ...
    test("should migrate multiple skills in different categories", async () => {
      ...
      await writeFile(
        join(currentSkillsDir, "tailwindcss", "skill.md"),
        "tailwind",
      );
      ...
      await mkdir(join(newSkillsDir, "web-frameworks", "nextjs"), {
        recursive: true,
      });
      await writeFile(
        join(newSkillsDir, "web-frameworks", "nextjs", "skill.md"),
        "nextjs",
      );
      await mkdir(join(newSkillsDir, "ui-styling", "tailwindcss"), {
        recursive: true,
      });
      await writeFile(
        join(newSkillsDir, "ui-styling", "tailwindcss", "skill.md"),
        "tailwind",
      );
      ...
      expect(
        await pathExists(join(currentSkillsDir, "web-frameworks", "nextjs")),
      ).toBe(true);
      expect(
        await pathExists(join(currentSkillsDir, "ui-styling", "tailwindcss")),
      ).toBe(true);
    });
  });
  ...
  describe("migrate - backup functionality", () => {
    test("should create backup when requested", async () => {
      ...
      if (result.backupPath) {
        ...
        expect(await pathExists(join(result.backupPath, "gemini-vision"))).toBe(
          true,
        );
      }
    });
  ...
  describe("migrate - edge cases", () => {
    ...
    test("should handle skills with binary files", async () => {
      ...
      expect(
        await pathExists(
          join(currentSkillsDir, "ai-multimodal", "gemini-vision", "image.png"),
        ),
      ).toBe(true);
    });
  });
