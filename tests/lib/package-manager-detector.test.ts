...
describe("PackageManagerDetector", () => {
  ...
  beforeEach(() => {
    ...
    testHomeDir = join(
      tmpdir(),
      `ck-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe("detect", () => {
    test("detects bun from npm_config_user_agent", async () => {
      process.env.npm_config_user_agent =
        "bun/1.3.2 npm/? node/v22.11.0 linux x64";
    ...
    test("detects yarn from npm_config_user_agent", async () => {
      process.env.npm_config_user_agent =
        "yarn/1.22.19 npm/? node/v20.9.0 linux x64";
      const pm = await PackageManagerDetector.detect();
      expect(pm).toBe("yarn");
    });
    ...
    test("detects pnpm from npm_config_user_agent", async () => {
      process.env.npm_config_user_agent =
        "pnpm/8.10.0 npm/? node/v20.9.0 linux x64";
      const pm = await PackageManagerDetector.detect();
      expect(pm).toBe("pnpm");
    });
  ...
  describe("getInstallCommand", () => {
    test("returns same as update command", () => {
      ...
      const installCmd = PackageManagerDetector.getInstallCommand(
        "npm",
        "pkg",
        "1.0.0",
      );
  ...
  describe("clearCache", () => {
    test("removes cache file when exists", async () => {
      ...
      writeFileSync(
        cacheFile,
        JSON.stringify({ packageManager: "bun", detectedAt: Date.now() }),
      );
      expect(existsSync(cacheFile)).toBe(true);
  ...
  describe("edge cases", () => {
    ...
    test("handles cache boundary - exactly 30 days old", async () => {
      ...
      writeFileSync(
        join(cacheDir, "install-info.json"),
        JSON.stringify(exactlyThirtyDays),
      );
  ...
  describe("security - input validation", () => {
    ...
    test("rejects invalid version", () => {
      expect(() => {
        PackageManagerDetector.getUpdateCommand(
          "npm",
          "valid-pkg",
          "1.0.0; rm -rf /",
        );
