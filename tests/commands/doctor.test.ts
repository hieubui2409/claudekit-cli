...
describe("Doctor Command", () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(
      join(mockClaudeDir, "agents", "agent1.md"),
      "# Agent 1",
      "utf8",
    );
    await writeFile(
      join(mockClaudeDir, "agents", "agent2.md"),
      "# Agent 2",
      "utf8",
    );
    ...
    await writeFile(
      join(mockClaudeDir, "commands", "command1.md"),
      "# Command 1",
      "utf8",
    );
    await writeFile(
      join(mockClaudeDir, "commands", "command2.md"),
      "# Command 2",
      "utf8",
    );
    await writeFile(
      join(mockClaudeDir, "commands", "command3.md"),
      "# Command 3",
      "utf8",
    );
    ...
    await writeFile(
      join(mockClaudeDir, "workflows", "workflow1.md"),
      "# Workflow 1",
      "utf8",
    );
  ...
  describe("doctorCommand", () => {
    ...
    test("should run without errors outside project directory", async () => {
      const nonProjectDir = join(
        process.cwd(),
        "test-temp",
        `non-project-${Date.now()}`,
      );
  ...
  describe("Component counting logic", () => {
    test("should count only .md files in agents directory", async () => {
      // Add a non-md file
      await writeFile(
        join(mockClaudeDir, "agents", "readme.txt"),
        "Readme",
        "utf8",
      );
