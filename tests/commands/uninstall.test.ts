...
describe("uninstall command integration", () => {
  ...
  describe("manifest-based uninstall", () => {
    test("should use manifest for accurate file removal", async () => {
      ...
      await writeFile(
        join(testLocalClaudeDir, "commands", "test.md"),
        "command",
      );
      await writeFile(join(testLocalClaudeDir, "skills", "skill1.md"), "skill");
      await writeFile(
        join(testLocalClaudeDir, "agents", "researcher.md"),
        "agent",
      );
      ...
      const { uninstallCommand } =
        await import("../../src/commands/uninstall/index.js");
      ...
      expect(existsSync(join(testLocalClaudeDir, "commands", "test.md"))).toBe(
        false,
      );
      expect(existsSync(join(testLocalClaudeDir, "skills", "skill1.md"))).toBe(
        false,
      );
      expect(
        existsSync(join(testLocalClaudeDir, "agents", "researcher.md")),
      ).toBe(false);
    ...
    test("should preserve custom user config files from manifest", async () => {
      ...
      await writeFile(
        join(testLocalClaudeDir, "my-custom-config.json"),
        '{"custom": true}',
      );
      ...
      expect(
        existsSync(join(testLocalClaudeDir, "my-custom-config.json")),
      ).toBe(true);
    });
  });
  ...
  describe("scope selection", () => {
    test("should handle local scope flag", async () => {
      ...
      await writeFile(
        join(testGlobalClaudeDir, "commands", "global-test.md"),
        "global",
      );
      await writeFile(
      ...
      expect(
        existsSync(join(testGlobalClaudeDir, "commands", "global-test.md")),
      ).toBe(true);
    });
