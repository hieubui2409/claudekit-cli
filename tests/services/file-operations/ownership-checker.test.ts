...
describe("OwnershipChecker", () => {
  ...
  describe("calculateChecksum", () => {
    ...
    test("throws on missing file", async () => {
      ...
      await expect(
        OwnershipChecker.calculateChecksum(missingFile),
      ).rejects.toThrow();
    });
  });
  ...
  describe("checkOwnership", () => {
    test("returns 'user' when no metadata", async () => {
      ...
      const result = await OwnershipChecker.checkOwnership(
        testFile,
        null,
        tempDir,
      );
    ...
    test("returns 'ck-modified' when checksum differs", async () => {
      ...
      const originalChecksum =
        await OwnershipChecker.calculateChecksum(testFile);
  ...
  describe("checkBatch", () => {
    test("processes multiple files in parallel", async () => {
      ...
      const results = await OwnershipChecker.checkBatch(
        [file1, file2],
        metadata,
        tempDir,
      );
