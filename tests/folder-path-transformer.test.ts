...
describe("Folder Path Transformer", () => {
  ...
  beforeEach(async () => {
    testDir = join(
      tmpdir(),
      `ck-folder-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe("transformFolderPaths", () => {
    ...
    test("should rename both folders", async () => {
      ...
      expect(await pathExists(join(testDir, "implementation-plans"))).toBe(
        true,
      );
    });
...
Link: [docs](docs/)
    ...
    test("should handle target folder already exists (EEXIST scenario)", async () => {
      ...
      expect(await pathExists(join(testDir, "ck-docs", "existing.md"))).toBe(
        true,
      );
    });
