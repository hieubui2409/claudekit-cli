...
import {
  type SettingsJson,
  SettingsMerger,
} from "../../../src/domains/config/settings-merger.js";
...
describe("SettingsMerger", () => {
  describe("merge", () => {
      ...
      const source: SettingsJson = {
        hooks: {
          SessionStart: [
            {
              type: "command",
              command: "node .claude/hooks/ck-session-start.cjs",
            },
          ],
        },
      };
    ...
    it("should not overwrite existing MCP server with same name", () => {
      ...
      expect(result.merged.mcp?.servers?.["shared-server"]?.command).toBe(
        "my-custom-server.js",
      );
    ...
    it("should merge hooks into existing matcher instead of creating duplicate (issue #219)", () => {
      ...
      expect(entry.hooks[1].command).toBe(
  ...
  describe("file operations", () => {
    ...
    it("should return null for non-existent file", async () => {
      const settings = await SettingsMerger.readSettingsFile(
    ...
    it("should atomically write settings file (no leftover temp files)", async () => {
      ...
      const tempFiles = files.filter(
        (f) => f.includes(".tmp") || f.includes(".settings-"),
      );
  ...
  describe("complex merge scenarios", () => {
    it("should handle real-world settings merge", () => {
          ...
          SessionStart: [
            {
              ...
              hooks: [
                {
                  type: "command",
                  command: 'node "$HOME"/.claude/hooks/session-start.cjs',
                },
              ],
            },
          ],
          UserPromptSubmit: [
            {
              ...
              hooks: [
                {
                  type: "command",
                  command: 'node "$HOME"/.claude/hooks/user-prompt-submit.cjs',
                },
              ],
            },
          ],
      ...
      expect(sessionStart.hooks[1].command).toBe(
        'node "$HOME"/.claude/hooks/session-start.cjs',
      );
  ...
  describe("malformed hook entries", () => {
    it("should handle hook entry without command field", () => {
      const source: SettingsJson = {
        hooks: {
          SessionStart: [
            { type: "command" } as unknown as { type: string; command: string },
          ],
        },
      };
  ...
  describe("hook execution order", () => {
    it("should place user hooks before CK hooks (user priority)", () => {
      ...
      const hooks = result.merged.hooks?.SessionStart as Array<{
        command: string;
      }>;
  ...
  describe("respect user deletions", () => {
    ...
    it("should add hooks that user never had", () => {
      ...
      expect(result.merged.hooks?.SessionStart).toHaveLength(2);
      expect(result.newlyInstalledHooks).toContain(
        "node .claude/hooks/new-hook.cjs",
      );
    });
    ...
    it("should track newly installed hooks", () => {
      const source: SettingsJson = {
        hooks: {
          SessionStart: [
            { type: "command", command: "node .claude/hooks/brand-new.cjs" },
          ],
        },
      };
      ...
      expect(result.newlyInstalledHooks).toContain(
        "node .claude/hooks/brand-new.cjs",
      );
    });
