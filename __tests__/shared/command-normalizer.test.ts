...
describe("normalizeCommand", () => {
  ...
  describe("$CLAUDE_PROJECT_DIR normalization", () => {
    it("should normalize unquoted $CLAUDE_PROJECT_DIR to $HOME", () => {
      expect(
        normalizeCommand("node $CLAUDE_PROJECT_DIR/.claude/hooks/init.js"),
      ).toBe("node $HOME/.claude/hooks/init.js");
    });
    ...
    it("should normalize multiple occurrences", () => {
      expect(
        normalizeCommand("$CLAUDE_PROJECT_DIR/a && $CLAUDE_PROJECT_DIR/b"),
      ).toBe("$HOME/a && $HOME/b");
    });
  });
  ...
  describe("Windows path normalization", () => {
    ...
    it("should normalize quoted %USERPROFILE%", () => {
      expect(
        normalizeCommand('node "%USERPROFILE%"/.claude/hooks/init.js'),
      ).toBe("node $HOME/.claude/hooks/init.js");
    });
  ...
  describe("real-world hook command scenarios", () => {
    it("should normalize global install hook", () => {
      ...
      expect(normalized).toBe(
        "node $HOME/.claude/hooks/session-start.cjs compact",
      );
    });

    it("should normalize local install hook to match global", () => {
      const localHook =
        'node "$CLAUDE_PROJECT_DIR"/.claude/hooks/session-start.cjs compact';
